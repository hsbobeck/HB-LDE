...
static void obs_x264_defaults(obs_data_t *settings)
{
	...
	obs_data_set_default_string(settings, "profile", "main"); // NOTE: Default to the "main" profile (was "high").
	obs_data_set_default_string(settings, "tune", "zerolatency");
	obs_data_set_default_string(settings, "x264opts", "scenecut=0 bframes=0");
			    ...
			    bool update)
{
	...
	char *tune = bstrdup("zerolatency"/*obs_data_get_string(settings, "tune")*/);
