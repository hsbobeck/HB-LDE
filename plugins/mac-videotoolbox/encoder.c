				    ...
				    int max_bitrate, double max_bitrate_window)
{
	...
	if (strcmp(rate_control, "CBR") == 0) {
		...
		if (__builtin_available(macOS 14.0, *)) { // NOTE: Was 13.0.
			if (is_apple_silicon) {
				compressionPropertyKey =
...
static bool update_params(struct vt_encoder *enc, obs_data_t *settings)
{
    ...
    // NOTE: We want bframes to always be zero/disabled.
    enc->bframes = false; // obs_data_get_bool(settings, "bframes");
...
static void vt_defaults(obs_data_t *settings, void *data)
{
	...
	obs_data_set_default_int(settings, "keyint_sec", 1); // NOTE: Default to 1 (originally was 2).
    // NOTE: Default to the "main" profile.
	...
	obs_data_set_default_int(settings, "codec_type",
				 kCMVideoCodecType_AppleProRes422);
    // NOTE: We want bframes to always be zero/disabled.
	obs_data_set_default_bool(settings, "bframes", false);
}
