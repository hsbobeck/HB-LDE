...
static void CALLBACK post_tick(PVOID param, BOOLEAN)
{
	...
	PostThreadMessageW(static_cast<DWORD>(worker_tid), MixerEvents::Tick, NULL, NULL);
}
...
Mixer::~Mixer()
{
	auto unusedDeleteResult = DeleteTimerQueueTimer(NULL, timer, INVALID_HANDLE_VALUE);
	(void)unusedDeleteResult;
