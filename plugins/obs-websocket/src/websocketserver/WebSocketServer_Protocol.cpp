				     ...
				     WebSocketOpCode::WebSocketOpCode opCode, json &payloadData)
{
	...
	case WebSocketOpCode::Identify: { // Identify
		...
		if (session->AuthenticationRequired()) {
			...
			if (!Utils::Crypto::CheckAuthenticationString(session->Secret(), session->Challenge(),
				...
				if (conf && conf->AlertsEnabled) {
					QString title = obs_module_text("LDEWebSocket.TrayNotification.AuthenticationFailed.Title");
					QString body =
						QString(obs_module_text("LDEWebSocket.TrayNotification.AuthenticationFailed.Body"))
		...
		auto conf = GetConfig();
		if (conf && conf->AlertsEnabled) {
			QString title = obs_module_text("LDEWebSocket.TrayNotification.Identified.Title");
			QString body = QString(obs_module_text("LDEWebSocket.TrayNotification.Identified.Body"))
					       .arg(QString::fromStdString(session->RemoteAddress()));
