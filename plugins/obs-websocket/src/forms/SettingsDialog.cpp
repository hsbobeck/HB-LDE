...
void SettingsDialog::SaveFormData()
{
	...
	if (ui->serverPasswordLineEdit->text().length() < 6) {
		QMessageBox msgBox;
		msgBox.setWindowTitle(obs_module_text("LDEWebSocket.Settings.Save.PasswordInvalidErrorTitle"));
		msgBox.setText(obs_module_text("LDEWebSocket.Settings.Save.PasswordInvalidErrorMessage"));
	...
	if (passwordManuallyEdited && (conf->ServerPassword != ui->serverPasswordLineEdit->text())) {
		QMessageBox msgBox;
		msgBox.setWindowTitle(obs_module_text("LDEWebSocket.Settings.Save.UserPasswordWarningTitle"));
		msgBox.setText(obs_module_text("LDEWebSocket.Settings.Save.UserPasswordWarningMessage"));
		msgBox.setInformativeText(obs_module_text("LDEWebSocket.Settings.Save.UserPasswordWarningInfoText"));
	...
	conf->ServerEnabled = ui->enableWebSocketServerCheckBox->isChecked();
	conf->AlertsEnabled = false; // ui->enableSystemTrayAlertsCheckBox->isChecked();
...
void SettingsDialog::FillSessionTable()
{
	...
	QString kickButtonText = obs_module_text("LDEWebSocket.SessionTable.KickButtonText");
...
void SettingsDialog::ShowConnectInfoButtonClicked()
{
	if (obs_video_active()) {
		QMessageBox msgBox;
		msgBox.setWindowTitle(obs_module_text("LDEWebSocket.Settings.ShowConnectInfoWarningTitle"));
		msgBox.setText(obs_module_text("LDEWebSocket.Settings.ShowConnectInfoWarningMessage"));
		msgBox.setInformativeText(obs_module_text("LDEWebSocket.Settings.ShowConnectInfoWarningInfoText"));
		msgBox.setStandardButtons(QMessageBox::Yes | QMessageBox::No);
		msgBox.setDefaultButton(QMessageBox::No);
		int ret = msgBox.exec();
