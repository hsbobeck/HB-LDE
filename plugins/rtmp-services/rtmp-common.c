...
static void rtmp_common_update(void *data, obs_data_t *settings)
{
    ...
    const char* origServer = obs_data_get_string(settings, "server");
    const char* origKey = obs_data_get_string(settings, "key");
    //
    char* finalServer = NULL;
    char* finalKey = NULL;
    //
    LdeProvideRtmpServerKey(origServer, origKey, &finalServer, &finalKey);
	...
	service->server = finalServer;
	service->key = finalKey;
...
static json_t *open_services_file(void)
{
	...
	file = obs_module_config_path("services-no.json");
	if (file) {
	...
	if (!root) {
		file = obs_module_file("services-no.json");
		if (file) {
			root = open_json_file(file);
			bfree(file);
		}
					 ...
					 json_t *recommended)
{
    ...
    // NOTE: We want bframes to always be 0.
