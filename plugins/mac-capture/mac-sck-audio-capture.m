...
static void sck_audio_capture_defaults(obs_data_t *settings)
{
    ...
    obs_data_set_default_int(settings, "type", ScreenCaptureAudioApplicationStream);
}
                                         ...
                                         obs_data_t *settings)
{
    ...
    switch (capture_type_id) {
        ...
        case ScreenCaptureAudioApplicationStream: {
            ...
            build_application_list(sc, props, /*audioList*/ true);
            break;
        }
    }
...
static bool LdeOpenSystemSettings(obs_properties_t *props __unused, obs_property_t *property __unused, void *data __unused)
{
...
static obs_properties_t *sck_audio_capture_properties(void *data)
{
    ...
    bool permissionGranted = CGPreflightScreenCaptureAccess();

    if (permissionGranted) {
        obs_property_t *capture_type = obs_properties_add_list(props, "type", obs_module_text("SCK.Method"),
                                                               OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);

        obs_property_list_add_int(capture_type, obs_module_text("ApplicationAudioCapture"), ScreenCaptureAudioApplicationStream);
        ...
        obs_property_t *permissionsError = obs_properties_add_text(
            props, "lde_permissions_error", obs_module_text("Permission not granted"), OBS_TEXT_INFO);
        obs_property_text_set_info_type(permissionsError, OBS_TEXT_INFO_ERROR);

        obs_properties_add_button(props, "lde_open_system_settings",
                                  obs_module_text("SCK.LdeOpenSystemSettings"),
                                  LdeOpenSystemSettings);
    }
