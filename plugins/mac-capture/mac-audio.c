...
// NOTE: These should be written only once, when the plugin is loaded, and then read as needed. This simple approach should be good enough.
static int sLouperVirtualAudio2ch_Status = -1; // NOTE: -1: Unknown, 0 - Absent, 1 - Present.
static char* sLouperVirtualAudio2ch_Value = NULL;
static char* sLouperVirtualAudio2ch_Name = NULL;

bool Is_LouperVirtualAudio2ch_Available()
{
    if (sLouperVirtualAudio2ch_Status != -1) {
        return (sLouperVirtualAudio2ch_Status == 1);
    }

    struct device_list devices;
    memset(&devices, 0, sizeof(struct device_list));
    coreaudio_enum_devices(&devices, /*input*/ true);

    sLouperVirtualAudio2ch_Status = 0;
    //
    bfree(sLouperVirtualAudio2ch_Name);
    sLouperVirtualAudio2ch_Name = NULL;
    //
    bfree(sLouperVirtualAudio2ch_Value);
    sLouperVirtualAudio2ch_Value = NULL;

    for (size_t i = 0; i < devices.items.num; i++) {
        struct device_item *item = devices.items.array + i;

        if ((0 == strcmp("Louper LDEConnect 2ch", item->name.array)) || (0 == strcmp("Louper LBSConnect 2ch", item->name.array))) {
            sLouperVirtualAudio2ch_Status = 1;
            sLouperVirtualAudio2ch_Value = bstrdup(item->value.array);
            sLouperVirtualAudio2ch_Name = bstrdup(item->name.array);
            break;
        }
    }
...
static const char *louper_virtual_audio_2ch_input_getname(void *unused)
{
    UNUSED_PARAMETER(unused);
    return sLouperVirtualAudio2ch_Name;
}

static void *louper_virtual_audio_2ch_create_input_capture(obs_data_t *settings, obs_source_t *source)
{
    return coreaudio_create(settings, source, /*input*/ true);
}

static void louper_virtual_audio_2ch_destroy(void *data)
{
    return coreaudio_destroy(data);
}

static void louper_virtual_audio_2ch_update(void *data, obs_data_t *settings)
{
    // NOTE: Since there are no properties, there's nothing to update here.
    UNUSED_PARAMETER(data);
    UNUSED_PARAMETER(settings);
}

static void louper_virtual_audio_2ch_defaults(obs_data_t *settings)
{
    obs_data_set_default_string(settings, "device_id", sLouperVirtualAudio2ch_Value);
}
...
struct obs_source_info louper_virtual_audio_2ch_input_capture_info = {
    .id = "louper_virtual_audio_2ch_input_capture",
    .type = OBS_SOURCE_TYPE_INPUT,
    .output_flags = OBS_SOURCE_AUDIO | OBS_SOURCE_DO_NOT_DUPLICATE,
    .get_name = louper_virtual_audio_2ch_input_getname,
    .create = louper_virtual_audio_2ch_create_input_capture,
    .destroy = louper_virtual_audio_2ch_destroy,
    .update = louper_virtual_audio_2ch_update,
    .get_defaults = louper_virtual_audio_2ch_defaults,
    .get_properties = NULL,
    .icon_type = OBS_ICON_TYPE_AUDIO_INPUT,
};
