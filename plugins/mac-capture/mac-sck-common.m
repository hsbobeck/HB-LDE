...
static bool IsAppNamePreferred(NSSet* preferredAppNameSet, NSString* queryAppName)
{
    for (NSString* preferredAppName in preferredAppNameSet) {
        if ([queryAppName containsString:preferredAppName]) {
            return true;
        }
    }
    return false;
...
bool build_application_list(struct screen_capture *sc, obs_properties_t *props, bool audioList)
{
    os_sem_wait(sc->shareable_content_available);
    ...
    filteredApplications = [sc->shareable_content.applications
        filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(SCRunningApplication *app,
            ...
            if (audioList) {
                if ([app.bundleIdentifier isEqualToString:@"io.louper.lde2"]) {
                    return false;
                }
            }
    ...
    static NSSet* sPreferredAppNameSet = nil;
    if (audioList) {
        if (!sPreferredAppNameSet) {
            sPreferredAppNameSet = [[NSSet setWithObjects:
                                     @"Ableton",
                                     @"After Effects",
                                     @"Avid",
                                     @"Final Cut",
    ...
    NSMutableDictionary* nameToPreferredNumberMap = nil;
    int preferredAppCount = 0;
    if (audioList) {
        nameToPreferredNumberMap = [[[NSMutableDictionary alloc] init] autorelease];
        for (SCRunningApplication* scApp in filteredApplications) {
            NSString* name = scApp.applicationName;
            BOOL preferredFlag = IsAppNamePreferred(sPreferredAppNameSet, name) ? YES : NO;
            NSNumber* preferredNumber = [NSNumber numberWithBool:preferredFlag];
            [nameToPreferredNumberMap setObject:preferredNumber forKey:name];
            if (preferredFlag) {
                preferredAppCount += 1;
            }
        }
    ...
    sortedApplications = [filteredApplications
        sortedArrayUsingComparator:^NSComparisonResult(SCRunningApplication* scAppA, SCRunningApplication* scAppB) {
            NSString* nameA = scAppA.applicationName;
            NSString* nameB = scAppB.applicationName;

            if (audioList) {
                bool preferredA = [[nameToPreferredNumberMap objectForKey:nameA] boolValue];
                bool preferredB = [[nameToPreferredNumberMap objectForKey:nameB] boolValue];
    ...
    for (SCRunningApplication *application in sortedApplications) {
        if (audioList && (preferredAppCount > 0)) {
            if (appIndex == preferredAppCount) {
    ...
    if (audioList && (preferredAppCount > 0)) {
        obs_property_list_item_disable(application_list, preferredAppCount, true);
    }
