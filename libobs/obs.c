...
static void LdeProvideRtmpServerKey_Fallback(const char* url, char** serverPtr, char** keyPtr)
{
    *serverPtr = bstrdup(url);
    *keyPtr = bstrdup("");
}

static void LdeProvideRtmpServerKey_Core(const char* url, char** serverPtr, char** keyPtr)
{
    const char* p = url;
    ...
    {
        const char* kNeedle = "://";
        const size_t kNeedleLength = 3;

        p = strstr(p, kNeedle);
        if (!p) {
            return LdeProvideRtmpServerKey_Fallback(url, serverPtr, keyPtr);
        }
        p += kNeedleLength;
    }

    // Find post-host/port slash
    {
        const char* kNeedle = "/";
        const size_t kNeedleLength = 1;
    ...
    // Find the first slash before the key marker
    // NOTE: There is at least the post-host/port slash.
    while (*p != '/') {
        p -= 1;
    }

    // NOTE: We exclude the slash at *p from the server part, even if it is the post-host/port slash
    *serverPtr = bstrdup_n(url, (p - url));
    //
    *keyPtr = bstrdup(p + 1);
}

// NOTE: The resulting *finalServerPtr and *finalKeyPtr must be bfree()'d.
void LdeProvideRtmpServerKey(const char* origServer, const char* origKey, char** finalServerPtr, char** finalKeyPtr)
{
    bool origServerEmpty = ((origServer == NULL) || (origServer[0] == '\0'));
    bool origKeyEmpty = ((origKey == NULL) || (origKey[0] == '\0'));

    if (origServerEmpty) {
        if (origKeyEmpty) {
            return LdeProvideRtmpServerKey_Core("", finalServerPtr, finalKeyPtr);
        } else {
            return LdeProvideRtmpServerKey_Core(origKey, finalServerPtr, finalKeyPtr);
        }
    } else {
        if (origKeyEmpty) {
            return LdeProvideRtmpServerKey_Core(origServer, finalServerPtr, finalKeyPtr);
        } else {
            *finalServerPtr = bstrdup(origServer);
            *finalKeyPtr = bstrdup(origKey);
        }
    }
}
