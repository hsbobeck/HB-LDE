...
#ifdef __APPLE__
        bool screenCapture = (strcmp(source->info.id, "screen_capture") == 0);
        bool desktopAudio = (strcmp(source->info.id, "sck_audio_capture") == 0);

        if (screenCapture || desktopAudio) {
            // NOTE: For Mac Screen Capture and Desktop Audio, when Screen Recording permissions are not available, we show the corresponding UI in properties.
        } else {
            return NULL;
        }
#else
