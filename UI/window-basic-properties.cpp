...
OBSBasicProperties::OBSBasicProperties(QWidget *parent, OBSSource source_, bool deleteSourceOnCancel_, obs_sceneitem_t *deleteSourceSceneitem_)
	: QDialog(parent),
	  ui(new Ui::OBSBasicProperties),
	  main(qobject_cast<OBSBasic *>(parent)),
	  acceptClicked(false),
	  source(source_),
      deleteSourceOnCancel(deleteSourceOnCancel_),
      deleteSourceSceneitem(deleteSourceSceneitem_),
...
OBSBasicProperties::~OBSBasicProperties()
{
    ...
    if (doDeleteSource) {
        obs_sceneitem_remove(deleteSourceSceneitem);
    }
