...
#define LDE_VIDEO_AUDIO_CHANGED &OBSBasicSettings::LdeVideoAudioChanged // NOTE: This macro is currently unused.
/* clang-format on */

OBSBasicSettings::OBSBasicSettings(QWidget *parent, int initialPageIndex)
	: QDialog(parent),
	  ...
	  ui(new Ui::OBSBasicSettings)
{
    ...
    ui->ldePreferences_SysTray_GroupBox->setVisible(false);
    ui->ldePreferences_StudioMode_GroupBox->setVisible(false);
    ui->bitrateGroupBox->setVisible(false);
	...
	HookWidget(ui->baseResolution,       COMBO_CHANGED,  VIDEO_RES);
	HookWidget(ui->outputResolution,     COMBO_CHANGED,  VIDEO_RES);
    ...
    HookWidget(ui->ldeAutoVolumeReduction, CHECK_CHANGED, AUDIO_CHANGED);
#ifdef _WIN32
    ...
    ui->fpsCommon->addItem("23.976", QVariant::fromValue<QString>("23.976"));
    ui->fpsCommon->addItem(QTStr("24"), QVariant::fromValue<QString>("24"));
    ui->fpsCommon->addItem(QTStr("25"), QVariant::fromValue<QString>("25"));
    ui->fpsCommon->addItem("30", QVariant::fromValue<QString>("30"));
    ui->fpsCommon->addItem(QTStr("50"), QVariant::fromValue<QString>("50"));
    ui->fpsCommon->addItem("60", QVariant::fromValue<QString>("60"));
    ui->fpsCommon->setCurrentIndex(0);
...
#undef ADD_HOTKEY_FOCUS_TYPE
...
#if 1 // !defined(_WIN32) && !defined(ENABLE_SPARKLE_UPDATER)
    ...
    ui->listWidget->setCurrentRow(initialPageIndex);
    ui->settingsPages->setCurrentIndex(initialPageIndex);

    if (0 == initialPageIndex) {
        ui->key->selectAll();
        QTimer::singleShot(0, ui->key, SLOT(setFocus()));
    }
}
				       ...
				       bool ignoreAllSignals)
{
    ...
    bool lockedOutputRes = false; // !ui->outputResolution->isEditable();

	if (!lockedOutputRes) {
        // NOTE: Disabled as unnecessary.
#if 0
	...
	if (!out_cx || !out_cy) {
		out_cx = cx;
		out_cy = cy;
        oldOutputRes = ResString(cx, cy).c_str();
	} else {
		oldOutputRes =
...
{
    int cx = config_get_uint(main->Config(), "Video", "BaseCX");
    int cy = config_get_uint(main->Config(), "Video", "BaseCY");

    auto resetSourceResolution = false;
    auto resetOutputResolution = false;

    {
        std::vector<std::pair<int, int>> resolutions{};
        resolutions.push_back(std::pair<int, int>(960, 540));
        resolutions.push_back(std::pair<int, int>(1280, 720));
        //
        auto defaultIndex = static_cast<int>(resolutions.size());
        resolutions.push_back(std::pair<int, int>(1920, 1080));
        //
        resolutions.push_back(std::pair<int, int>(2560, 1440));
        resolutions.push_back(std::pair<int, int>(3840, 2160));

        auto query = std::pair<int, int>(cx, cy);
        auto findResult = std::find(resolutions.begin(), resolutions.end(), query);

        ui->baseResolution->clear();
        for (auto& pair: resolutions) {
            QString res = ResString(pair.first, pair.second).c_str();
            ui->baseResolution->addItem(res);
        }

        if (findResult != resolutions.end()) {
            auto index = std::distance(resolutions.begin(), findResult);
            ui->baseResolution->setCurrentIndex(index);
        } else {
            ui->baseResolution->setCurrentIndex(defaultIndex);
            resetSourceResolution = true;
        }
    }

    int out_cx = config_get_uint(main->Config(), "Video", "OutputCX");
    int out_cy = config_get_uint(main->Config(), "Video", "OutputCY");

    {
        ...
        auto query = std::pair<int, int>(out_cx, out_cy);
        auto findResult = std::find(resolutions.begin(), resolutions.end(), query);

        ui->outputResolution->clear();
        for (auto& pair: resolutions) {
            QString res = ResString(pair.first, pair.second).c_str();
            ui->outputResolution->addItem(res);
        }
        ...
        if (findResult != resolutions.end()) {
            ...
            ui->outputResolution->setCurrentIndex(defaultIndex);
            resetOutputResolution = true;
        }
    }

    if (resetSourceResolution || resetOutputResolution) {
        if (resetSourceResolution) {
            cx = 1920;
            cy = 1080;

            config_set_int(main->Config(), "Video", "BaseCX", cx);
            config_set_int(main->Config(), "Video", "BaseCY", cy);
        }
        if (resetOutputResolution) {
            out_cx = 1920;
            out_cy = 1080;

            config_set_int(main->Config(), "Video", "OutputCX", out_cx);
            config_set_int(main->Config(), "Video", "OutputCY", out_cy);
        }
    }
...
static inline void LoadFPSCommon(OBSBasic *main, Ui::OBSBasicSettings *ui)
{
    ...
    int idx = ui->fpsCommon->findData(QVariant::fromValue(QString(val)));
    if (idx == -1) {
        idx = ui->fpsCommon->findData(QVariant::fromValue(QString("30")));
    }
    if (idx == -1) {
        idx = 0;
    }
...
void OBSBasicSettings::LoadAudioSettings()
{
    ...
    bool ldeAutoVolumeReductionFlag = config_get_bool(GetGlobalConfig(), "Audio",
                          "LdeAutoVolumeReduction");
    ui->ldeAutoVolumeReduction->setChecked(ldeAutoVolumeReductionFlag);
...
void OBSBasicSettings::LoadSettings(bool changedOnly)
{
    ...
    if (!changedOnly || ldeVideoAudioChanged)
        LoadLdeVideoAudioSettings();
}
...
void OBSBasicSettings::SaveVideoSettings()
{
	...
	SaveComboData(ui->fpsCommon, "Video", "FPSCommon");
...
void OBSBasicSettings::SaveAudioSettings()
{
    ...
    if (WidgetChanged(ui->ldeAutoVolumeReduction)) {
        bool ldeAutoVolumeReductionFlag = ui->ldeAutoVolumeReduction->isChecked();
        config_set_bool(GetGlobalConfig(), "Audio", "LdeAutoVolumeReduction", ldeAutoVolumeReductionFlag);
    }
...
void OBSBasicSettings::SaveSettings()
{
    ...
    if (ldeVideoAudioChanged)
        SaveLdeVideoAudioSettings();
	...
	if (Changed()) {
        ...
        if (ldeVideoAudioChanged)
            AddChangedVal(changed, "lde video audio");
    ...
    if (!serverUrl.isEmpty()) {
        ...
        if (!protocolKindQ) {
            QMessageBox::critical(
                this,
                QTStr("LdeBadStreamKeyProtocolAlert.Title"),
                QTStr("LdeBadStreamKeyProtocolAlert.Message"),
...
void OBSBasicSettings::on_baseResolution_currentIndexChanged(int index)
{
    (void)index;
...
void OBSBasicSettings::on_outputResolution_currentIndexChanged(int index)
{
    (void)index;

    if (!loading) {
        QString text = ui->outputResolution->currentText();
...
void OBSBasicSettings::LdeVideoAudioChanged()
{
    if (!loading) {
        ldeVideoAudioChanged = true;
        sender()->setProperty("changed", QVariant(true));
        EnableApplyButton(true);
    }
}
...
QIcon OBSBasicSettings::GetLdeVideoAudioIcon() const
{
    return ldeVideoAudioIcon;
}

QIcon OBSBasicSettings::GetLdeSupportIcon() const
{
    return ldeSupportIcon;
}
...
void OBSBasicSettings::SetAccessibilityIcon(const QIcon &/*icon*/)
{
    ...
    ui->listWidget->item(8)->setHidden(true);
}

void OBSBasicSettings::SetAdvancedIcon(const QIcon &/*icon*/)
{
	// ui->listWidget->item(9)->setIcon(icon);
	ui->listWidget->item(9)->setHidden(true);
}

void OBSBasicSettings::SetLdeVideoAudioIcon(const QIcon &icon)
{
    ui->listWidget->item(1)->setIcon(icon);
}

void OBSBasicSettings::SetLdeSupportIcon(const QIcon &icon)
{
...
void OBSBasicSettings::UpdateAdvNetworkGroup()
{
	...
	ui->dynBitrate->setVisible(false /*enabled*/);
