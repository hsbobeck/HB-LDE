...
private:
 	//LDE_version
 	int LDE_version_major = 1; int LDE_version_minor = 3; int LDE_version_patch = 1;
 	std::string Latest_LDE_version = "0";
 	bool auto_update_enabled = true;
 	bool skip_update_once = false;
 	bool skip_version = false;
 	std::string version_to_skip = "0";
 	std::string update_checking_link = "https://storage.louper.io/lde/version/index.html";

 	bool update_settings_changed = false;
    int ldeNextShowSettingsPageIndex = 0;
	...
	QPointer<OBSUpdate> updateWindow;
#ifdef YOUTUBE_ENABLED
	QPointer<YouTubeAppDock> youtubeAppDock;
	uint64_t lastYouTubeAppDockCreationTime = 0;
#endif
	QPointer<OBSAbout> about;
    QPointer<LdeAuthors> _ldeAuthors;
    QPointer<LdeLicense> _ldeLicense;
	QPointer<OBSMissingFiles> missDialog;
    QPointer<LdeLogUploader> _ldeLogUploader;
    ...
    bool ldeChangingVBitrateCombo{};
    bool ldeChangingABitrateCombo{};
    ...
    void UpdateLdeBitrateDockValues(bool force);
    void ApplyLdeVBitrateComboboxChange(int newVBitrate);
    void ApplyLdeABitrateComboboxChange(int newABitrate);
    void InitLdeBitrateDock();
	...
	std::string GetLastLDEVersion();
	bool WriteUpdateSettingsFile();
	std::map<std::string, std::string> ReadUpdateSettingsFile(std::string filePath);
	std::string FindUpdateSettingsValue(std::map<std::string, std::string>& values, std::string key);
	bool LoadUpdateSettings();
 	bool VerifyVersion(std::string input_version);
    ...
    auto HasLdeSourcePreviewProjector() -> bool;
    ...
    //obs_hotkey_pair_id recordingHotkeys, pauseHotkeys,
    ...
    int64_t reconnectGuard_lastReconnectSuccessUnixTimeQz;
    int reconnectGuard_extraDisconnectCount;

    void ReconnectGuard_Reset();
    void ShowTroubleshootStreamKeyAlert();
private slots:
    void ReconnectGuard_HandleDeactivate();
    void ReconnectGuard_HandleReconnectSuccess();

private:
    static void BgOBSOutputDeactivate(void *data, calldata_t *params);
    static void BgOBSOutputReconnectSuccess(void *data, calldata_t *params);

    auto LdeMakeWindowSizeKindFromScreen() -> LdeWindowSizeKind;
    auto LdeMakeWindowSizeKindFromCurrentWindowSize() -> LdeWindowSizeKind;
    auto LdeResizeWindow(LdeWindowSizeKind ldeWindowSizeKind) -> void;
    auto LdeFixSizeIfNeeded() -> void;
    auto LdeSaveGeometry() -> void;
...
private slots:
    ...
    void on_actionShowLdeAuthors_triggered();
    void on_actionShowLdeLicense_triggered();
...
#ifdef __APPLE__
    static auto Is_SourceId_LdeMacScreenCapture(const char *sourceId) -> bool;
    static auto Is_SourceId_LdeMacDesktopAudio(const char *sourceId) -> bool;
    auto Check_LdeMacScreenCaptureKitSource_PermissionsIfNeeded(const char *sourceId) -> bool /*allowedOrNa*/;
#endif
...
public:
    ...
    void CreatePropertiesWindow(obs_source_t *source, bool deleteSourceOnCancel = false, obs_sceneitem_t *deleteSourceSceneitem = nullptr);
...
private slots:
    ...
    void on_actionLdeUploadLogs_triggered();
    ...
    void on_streamKeyButton_clicked();
    ...
    void update_LdeSourcePreviewButton_BgColor();
    void on_ldeSourcePreviewButton_clicked();
...
public:
	...
	undo_stack undo_s;
    std::string LDEGetVersion() { return std::string{obs_get_version_string()}; }
