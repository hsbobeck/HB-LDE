...
static QString stream_settings_server_url(const Ui::OBSBasicSettings *ui)
{
...
//{
//    auto dataInt = ui->service->currentData().toInt();
//    return (static_cast<int>(ListOpt::RTMP) == dataInt)
//        || (static_cast<int>(ListOpt::SRT) == dataInt);
//}
//
...
//}
//
//static auto GetProtocolCStringForServiceListOpt(ListOpt serviceListOpt) -> const char*
//{
//    switch (serviceListOpt) {
//        case ListOpt::RTMP: return "RTMP";
//        case ListOpt::SRT: return "SRT";
//        case ListOpt::WHIP: return "WHIP";
//    }
//    return "";
//}
//
//static auto GetServiceListOptFromProtocolCString(const char* protocolCString) -> ListOpt
//{
//    if (protocolCString) {
//        if (strcmp("RTMP", protocolCString) == 0) {
//            return ListOpt::RTMP;
//        } else if (strcmp("SRT", protocolCString) == 0) {
//            return ListOpt::SRT;
//        } else if (strcmp("WHIP", protocolCString) == 0) {
//            return ListOpt::WHIP;
//        }
//    }
//    return ListOpt::RTMP;
...
void OBSBasicSettings::InitStreamPage()
{
    ...
    ui->moreInfoButton->setVisible(false);
    ui->getStreamKeyButton->setVisible(false);

    int vertSpacing = /*ui->topStreamLayout->verticalSpacing()*/0;
...
void OBSBasicSettings::LoadStream1Settings()
{
    ...
    //
    const char *protocolCString = obs_service_get_protocol(service_obj);
    protocol = QT_UTF8(protocolCString);
    //
    ...
    //auto serviceListOpt = GetServiceListOptFromProtocolCString(protocolCString);
    //ui->service->setCurrentIndex(static_cast<int>(serviceListOpt));
    ...
    if (is_whip) {
        ...
        char *finalServer = nullptr;
        char *finalKey = nullptr;
        LdeProvideRtmpServerKey(server, key, &finalServer, &finalKey);
        ui->customServer->setText(QT_UTF8(finalServer ? finalServer : ""));
        ui->key->setText(QT_UTF8(finalKey ? finalKey : ""));
        bfree(finalServer);
        bfree(finalKey);
    }
...
void OBSBasicSettings::LoadLdeVideoAudioSettings()
{
}

QString splitServerUrlKey(QString& server_url)
{
	QString output_key;
	if (server_url.contains("?policy="))
	{
		for (int i = server_url.size() - 1; i >= 0; i--)
		{
			if ((server_url[i] == '/' || server_url[i] == ' ') && i == server_url.size())
			{
				server_url.erase(server_url.begin() + i);
				continue;
			}
			else if (server_url[i] == '/')
			{
				server_url.erase(server_url.begin() + i);
				break;
			}
			output_key.push_front(server_url[i]);
			server_url.erase(server_url.begin() + i);
		}
	}
	return output_key;
...
{
    QString server_url = ui->customServer->text().trimmed();
    QString server_key = ui->key->text().trimmed();
    auto protocolKindQ = MakeProtocolKindQFromServerUrl(server_url);

    bool customServer = ((!protocolKindQ) || ((ProtocolKind::Rtmp == *protocolKindQ) || (ProtocolKind::Srt == *protocolKindQ)));
    bool whip = (protocolKindQ && (false /*(ProtocolKind::Whip == *protocolKindQ)*/));
    ...
    //if (server_url.startsWith("rtmp://") && server_key.isEmpty())
    //{
    //    server_key = splitServerUrlKey(server_url);
    //    ui->customServer->setText(server_url);
    //    ui->key->setText(server_key);
    //}
...
void OBSBasicSettings::SaveLdeVideoAudioSettings()
{
}
...
{
    QString serverUrl = stream_settings_server_url(ui.get());
    auto protocolKindQ = MakeProtocolKindQFromServerUrl(serverUrl);
    if (protocolKindQ) {
        switch (*protocolKindQ) {
            case ProtocolKind::Rtmp:
            case ProtocolKind::Srt: {
                ui->streamKeyLabel->setText(QTStr("Basic.AutoConfig.StreamPage.StreamKey"));
                break;
            }
            //case ProtocolKind::Whip: {
...
void OBSBasicSettings::LoadServices(bool /*showAll*/)
{
    ...
    //ui->service->clear();
    //ui->service->addItem(QTStr("RTMP"), QVariant((int)ListOpt::RTMP));
    //ui->service->addItem(QTStr("SRT"), QVariant((int)ListOpt::SRT));
	...
	ui->service->addItem(QTStr("RTMP"),
					QVariant((int)ListOpt::Custom));	// Keep ListOpt as "Custom" so that the Server field is editable

	ui->service->addItem(QTStr("SRT"),
				     QVariant((int)ListOpt::Custom));	// Keep ListOpt as "Custom" so that the Server field is editable
...
{
    bool external_oauth = false; // is_external_oauth(service);
	if (external_oauth) {
        ...
        bool can_auth = false; // is_auth_service(service);
...
void OBSBasicSettings::on_service_currentIndexChanged(int idx)
{
    ...
    // NOTE: Unused code.
#if 0
    auto serviceListOpt = static_cast<ListOpt>(idx);
    auto protocolCString = GetProtocolCStringForServiceListOpt(serviceListOpt);
    protocol = QT_UTF8(protocolCString);
    ...
    // NOTE: `protocol` is required for UpdateAdvNetworkGroup().
...
void OBSBasicSettings::ServiceChanged(bool resetFields)
{
    ...
    bool custom = ((!protocolKindQ) || ((ProtocolKind::Rtmp == *protocolKindQ) || (ProtocolKind::Srt == *protocolKindQ)));
    bool whip = (protocolKindQ && (false /*(ProtocolKind::Whip == *protocolKindQ)*/));
...
//						   ui->serverStackedWidget);
    ...
    bool service_check = false; //service.find(system_auth_service) != std::string::npos;
#ifdef YOUTUBE_ENABLED
...
auto OBSBasicSettings::MakeProtocolKindQFromServerUrl(const QString& origServerUrl) -> std::optional<ProtocolKind>
{
    auto finalServerUrl = origServerUrl.toLower();
    if (finalServerUrl.startsWith("rtmp://")) {
        return ProtocolKind::Rtmp;
    } else if (finalServerUrl.startsWith("srt://")) {
        return ProtocolKind::Srt;
    //} else if (finalServerUrl.startsWith("whip://")) {
    //    return ProtocolKind::Whip;
    }
    return {};
}
...
OBSService OBSBasicSettings::SpawnTempService()
{
	...
	if (whip)
		obs_data_set_string(settings, "bearer_token",
				    QT_TO_UTF8(ui->key->text().trimmed()));
	else
		obs_data_set_string(settings, "key",
				    QT_TO_UTF8(ui->key->text().trimmed()));
...
bool OBSBasicSettings::ServiceAndVCodecCompatible()
{
    QString serverUrl = stream_settings_server_url(ui.get());
    auto protocolKindQ = MakeProtocolKindQFromServerUrl(serverUrl);
    bool customService = ((!protocolKindQ) || ((ProtocolKind::Rtmp == *protocolKindQ) || (ProtocolKind::Srt == *protocolKindQ)));
