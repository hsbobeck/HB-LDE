				...
				const float inputPeak[MAX_AUDIO_CHANNELS])
{
    ...
    volControl->HandlePotentialDistortionPeaks(peak);
}
...
auto VolControl::ShowReducedSourceVolumeAlert(QString sourceName) -> void
{
    OBSMessageBox::information(this,
        QTStr("LdeReducedSourceVolumeAlert.Title"),
        QTStr("LdeReducedSourceVolumeAlert.MessageFormat").arg(sourceName));
}

auto VolControl::HandlePotentialDistortionPeaks(const float* peaks) -> void
{
    if (obs_source_muted(source)) {
        return;
    }

    auto distorted = false;
    //
    float maxDb = 0.0;
    float maxPeak = maxDb;
    //
    for (int channelIndex = 0; channelIndex < MAX_AUDIO_CHANNELS; channelIndex++) {
        float currentPeak = peaks[channelIndex];
        //
        if (!isfinite(currentPeak)) {
            continue;
        }
    ...
    bool ldeAutoVolumeReductionFlag = config_get_bool(GetGlobalConfig(), "Audio", "LdeAutoVolumeReduction");
    if (!ldeAutoVolumeReductionFlag) {
        return;
    }

    auto oldDb = obs_fader_get_db(obs_fader);
    auto newDb = oldDb - 10.0;
    obs_fader_set_db(obs_fader, newDb);
    //
    ...
    QMetaObject::invokeMethod(this, "ShowReducedSourceVolumeAlert", Qt::QueuedConnection,
        Q_ARG(QString, QString::fromUtf8(obs_source_get_name(source))));
}
