...
{
	const char *regKey =
		R"(Software\Microsoft\Windows\CurrentVersion\Uninstall\LDE)";
	LSTATUS res;
	HKEY key;
	char version[32];
	int formattedLen;
...
static bool Update(wchar_t *cmdLine)
{
	...
	StringCbCat(lpAppDataPath, sizeof(lpAppDataPath), L"\\Louper\\LDE");
	...
	if (!GetTempFileNameW(tempDirName, L"LDE", 0, tempPath)) {
...
static void LaunchOBS(LPWSTR lpCmdLine)
{
	...
	StringCbCat(obsPath, sizeof(obsPath), L"\\lde64.exe");
