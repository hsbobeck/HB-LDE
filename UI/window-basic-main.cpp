...
static void AddExtraModulePaths()
{
	...
	int ret = GetProgramDataPath(base_module_dir, sizeof(base_module_dir),
				     "Louper/LDE/plugins/%module%");
#elif defined(__APPLE__)
	int ret = GetConfigPath(base_module_dir, sizeof(base_module_dir),
				"Louper/LDE/plugins/%module%.plugin");
#else
	int ret = GetConfigPath(base_module_dir, sizeof(base_module_dir),
				"Louper/LDE/plugins/%module%");
#endif
...
static constexpr int kLdeWindowWidth = 400;
static constexpr int kLdeWindowHeight_Regular = 820;
//
static constexpr int kLde_ScenesSourcesDockHeight_Regular = 160;
static constexpr int kLde_ScenesSourcesDockHeight_Compact = 115;
//
static constexpr int kLde_MixerDockHeight_Regular = 160;
static constexpr int kLde_MixerDockHeight_Compact = 130;
//
static constexpr int kLdeWindowHeight_Compact =  kLdeWindowHeight_Regular
    - (kLde_ScenesSourcesDockHeight_Regular - kLde_ScenesSourcesDockHeight_Compact)
    - (kLde_MixerDockHeight_Regular - kLde_MixerDockHeight_Compact);

auto LdeGetWindowHeight(LdeWindowSizeKind ldeWindowSizeKind) -> int
{
    switch (ldeWindowSizeKind) {
        case LdeWindowSizeKind::Regular: { return kLdeWindowHeight_Regular; break; }
        case LdeWindowSizeKind::Compact: { return kLdeWindowHeight_Compact; break; }
    }
    return kLdeWindowHeight_Regular;
}
	  ...
	  ui(new Ui::OBSBasic)
{
	...
	splitDockWidget(ui->scenesDock, ui->mixerDock, Qt::Vertical);
	splitDockWidget(ui->scenesDock, ui->sourcesDock, Qt::Horizontal);
	splitDockWidget(ui->mixerDock, ui->ldeBitrateDock, Qt::Vertical);
	splitDockWidget(ui->ldeBitrateDock, ui->controlsDock, Qt::Vertical);
    ...
    ui->actionResetSettings->setMenuRole(
        QAction::ApplicationSpecificRole);
...
#undef SETUP_DOCK
    ...
    //// Get screen height and set the window's maximum height
    //int screenHeight = QGuiApplication::primaryScreen()->geometry().height();
    //setMaximumHeight(screenHeight);
    //
    auto ldeWindowSizeKind = LdeMakeWindowSizeKindFromScreen();
    LdeResizeWindow(ldeWindowSizeKind);
	...
	if (obs_audio_monitoring_available()) {
		...
		connect(ui->monitoringDevice, comboIndexChanged,
			[this](int index) {
				auto id = ui->monitoringDevice->itemData(index)
						  .toString()
						  .toUtf8();
				auto name =
					ui->monitoringDevice->itemText(index)
						.toUtf8();
				obs_set_audio_monitoring_device(
					name.constData(), id.constData());
				config_set_string(basicConfig, "Audio",
    ...
    update_LdeSourcePreviewButton_BgColor();
    
#ifdef __APPLE__
    do {
        if (!LdeIsLbsAppInstalled()) {
            break;
        }
        
        auto oldDoNotWarnAgain = config_get_bool(App()->GlobalConfig(), "BasicWindow", "Lde_LbsAppInstalled_DoNotWarnAgain");
        if (oldDoNotWarnAgain) {
            break;
        }
        
        QTimer::singleShot(1, [parent] {
            auto newDoNotWarnAgain = LdeShow_LbsAppInstalled_Alert();
            if (newDoNotWarnAgain) {
                config_set_bool(App()->GlobalConfig(), "BasicWindow", "Lde_LbsAppInstalled_DoNotWarnAgain", newDoNotWarnAgain);
            }
        });
    } while (false);
#endif
    
    ldeUpdater = new LdeUpdater(menuBar());

    LdeLogUploader::InitOnce();
    App()->SendMixpanelEvent(MixpanelEventKind::AppStarted, /*service*/ {}, /*sourceTreeModel*/ {});
}

// void OBSBasic::resizeEvent(QResizeEvent *event) {
//     QSize new_size = event->size();
//     float aspectRatio = 480.0f / 855.0f; // width / height

//     QRect screenRect = QGuiApplication::primaryScreen()->geometry();
//     int screenHeight = screenRect.height();

//     float deductionPercentage = 0.02; // 2% of screen height
//     int allowedHeight = screenHeight - y() - (screenHeight * deductionPercentage);

//     // Adjust height based on width while maintaining aspect ratio
//     int expectedHeight = new_size.width() / aspectRatio;

//     // If the expected height given the width exceeds the allowed height,
//     // adjust the width to maintain the aspect ratio
//     if (expectedHeight > allowedHeight) {
//         new_size.setWidth(allowedHeight * aspectRatio);
//         new_size.setHeight(allowedHeight);
//     } else if (new_size.height() > allowedHeight) {
//         // If only height exceeds the allowed height, adjust both width and height
//         new_size.setHeight(allowedHeight);
//         new_size.setWidth(allowedHeight * aspectRatio);
//     } else {
//         // If both dimensions are within limits, just maintain aspect ratio
//         if (new_size.width() / aspectRatio != new_size.height()) {
//             new_size.setHeight(new_size.width() / aspectRatio);
//         } else {
//             new_size.setWidth(new_size.height() * aspectRatio);
//         }
//     }
...
//     // Call base class method if you need default behavior
...
auto OBSBasic::LdeMakeWindowSizeKindFromScreen() -> LdeWindowSizeKind
{
    // NOTE: Compact window size kind is Mac-only at this point.
#ifdef __APPLE__
	auto screenHeightQz = LdeGetScreenAvailableHeightQz(this);
    if (0 == screenHeightQz) {
        return LdeWindowSizeKind::Regular;
    }
    if (screenHeightQz < kLdeWindowHeight_Regular) {
        return LdeWindowSizeKind::Compact;
    }
#endif
    return LdeWindowSizeKind::Regular;
}

auto OBSBasic::LdeMakeWindowSizeKindFromCurrentWindowSize() -> LdeWindowSizeKind
{
    auto windowHeight = size().height();
    if (windowHeight < kLdeWindowHeight_Regular) {
        return LdeWindowSizeKind::Compact;
    }
    return LdeWindowSizeKind::Regular;
}

auto OBSBasic::LdeResizeWindow(LdeWindowSizeKind ldeWindowSizeKind) -> void
{
    auto windowHeight = LdeGetWindowHeight(ldeWindowSizeKind);
    
    // NOTE: The patching in LdeSaveGeometry() is required for this (min size equals max size) to work.
    setMaximumSize(kLdeWindowWidth, windowHeight);
    setMinimumSize(kLdeWindowWidth, windowHeight);
    //
    resize(kLdeWindowWidth, windowHeight);
    
    int scenesSourcesDockHeight{};
    int mixerDockHeight{};
    //
    switch (ldeWindowSizeKind) {
        case LdeWindowSizeKind::Regular: {
            scenesSourcesDockHeight = kLde_ScenesSourcesDockHeight_Regular;
            mixerDockHeight = kLde_MixerDockHeight_Regular;
            break;
        }
        case LdeWindowSizeKind::Compact: {
            scenesSourcesDockHeight = kLde_ScenesSourcesDockHeight_Compact;
            mixerDockHeight = kLde_MixerDockHeight_Compact;
            break;
        }
    }
    //
    ui->scenesDock->setMinimumHeight(scenesSourcesDockHeight);
    ui->scenesDock->setMaximumHeight(scenesSourcesDockHeight);
    //
    ui->sourcesDock->setMinimumHeight(scenesSourcesDockHeight);
    ui->sourcesDock->setMaximumHeight(scenesSourcesDockHeight);
    //
    ui->mixerDock->setMaximumHeight(mixerDockHeight);
    ui->mixerDock->setMinimumHeight(mixerDockHeight);
}

auto OBSBasic::LdeFixSizeIfNeeded() -> void
{
    // NOTE: Mac-only for now.
#ifdef __APPLE__
    auto oldLdeWindowSizeKind = LdeMakeWindowSizeKindFromCurrentWindowSize();
    auto newLdeWindowSizeKind = LdeMakeWindowSizeKindFromScreen();
    if (oldLdeWindowSizeKind == newLdeWindowSizeKind) {
        return;
    }
    LdeResizeWindow(newLdeWindowSizeKind);
#endif
}

auto OBSBasic::LdeSaveGeometry() -> void
{
    auto byteArray = saveGeometry();

#ifdef __APPLE__
    // NOTE: Since we set the window size to be fixed (see the setMinimumSize()/setMaximumSize() calls in the constructor), saveGeometry() - for some QT-internal reason - produces an invalid `normalGeometry` part in the resulting serialized geometry (see the implementation of QWidget::saveGeometry()). The following is a fix.
    //
    auto geometryByteArray = byteArray.mid(50, 16);
    //
    // NOTE: Replace `normalGeometry` with `geometry`.
    byteArray.replace(24, geometryByteArray.size(), geometryByteArray);
    //
    // NOTE: We must keep the Maximized flag (bytes (44, 1)) unchanged, as otherwise the serialized geometry is considered invalid (again, for some QT-internal reason).
#endif

    config_set_string(
        App()->GlobalConfig(),
        "BasicWindow", "geometry",
        byteArray.toBase64().constData());
}
...
bool OBSBasic::InitService()
{
	...
	service = obs_service_create("rtmp_custom", nullptr, nullptr, nullptr);
	if (!service)
		return false;
	obs_service_release(service);
...
bool OBSBasic::InitBasicConfigDefaults()
{
	...
	config_set_default_uint(basicConfig, "Output", "DelaySec", 1);
	config_set_default_bool(basicConfig, "Output", "DelayPreserve", true);

	config_set_default_bool(basicConfig, "Output", "Reconnect", true);
	config_set_default_uint(basicConfig, "Output", "RetryDelay", 2); // NOTE: Originally 5; lowered for ReconnectGuard to kick in sooner.
	config_set_default_uint(basicConfig, "Output", "MaxRetries", 2);
	...
	config_set_default_uint(basicConfig, "Video", "OutputCX", cx); // scale_cx
	config_set_default_uint(basicConfig, "Video", "OutputCY", cy); // scale_cy
	...
	config_set_default_string(basicConfig, "Video", "FPSCommon", "25");
	config_set_default_uint(basicConfig, "Video", "FPSInt", 25);
	config_set_default_uint(basicConfig, "Video", "FPSNum", 25);
	...
	config_set_default_bool(basicConfig, "BasicWindow", "SideDocks", true);
...
void OBSBasic::InitBasicConfigDefaults2()
{
    ...
    //                      "Pre23Defaults");
	//bool useNV = false; // EncoderAvailable("ffmpeg_nvenc") && !oldEncDefaults;
	config_set_default_string(basicConfig, "SimpleOutput", "StreamEncoder",
#ifdef __APPLE__
        SIMPLE_ENCODER_APPLE_H264
#else
        SIMPLE_ENCODER_X264
#endif
    );
...
void OBSBasic::UpdateLdeBitrateDockValues(bool force)
{
    // NOTE: This is to prevent two extra bitrate changes (new -> old -> new) when showing an alert after a bitrate change (and thus prevent potential extra alert).
    if (!force) {
        if (ldeChangingVBitrateCombo || ldeChangingABitrateCombo) {
            return;
        }
    }
    ...
    auto videoCombo = ui->ldeBitrateDock_VideoCombo;
    auto audioCombo = ui->ldeBitrateDock_AudioCombo;

    auto newVIndex = videoCombo->findData(QVariant::fromValue(videoBitrate));
    if (newVIndex != -1) {
        auto oldVIndex = videoCombo->currentIndex();
        if (oldVIndex != newVIndex) {
            videoCombo->setCurrentIndex(newVIndex);
        }
    } else {
        // NOTE: We ignore unsupported vBitrate values and don't update the combo box to display them.
    }

    auto newAIndex = audioCombo->findData(QVariant::fromValue(audioBitrate));
    if (newAIndex != -1) {
        auto oldAIndex = audioCombo->currentIndex();
        if (oldAIndex != newAIndex) {
            audioCombo->setCurrentIndex(newAIndex);
        }
    } else {
        // NOTE: We ignore unsupported aBitrate values and don't update the combo box to display them.
    }
}

void OBSBasic::ApplyLdeVBitrateComboboxChange(int newVBitrate)
{
    // NOTE: Since a QComboBox fires currentIndexChanged only on an actual change, we don't compare the old and new vBitrates here.

    if (newVBitrate > 4000) {
        QMessageBox mb(QMessageBox::Warning,
                   QTStr("LdeHighVBitrateAlert.Title"),
                   QTStr("LdeHighVBitrateAlert.Message"));
        //
        auto cancelButton = mb.addButton(QTStr("Cancel"), QMessageBox::RejectRole);
        auto continueButton = mb.addButton(QTStr("Continue"), QMessageBox::AcceptRole);
        //
        mb.setDefaultButton(cancelButton);
        //
        mb.exec();
        auto continueFlag = (mb.clickedButton() == continueButton);

        if (!continueFlag) {
            UpdateLdeBitrateDockValues(/*force*/ true);
            return;
        }
    }
    ...
    if (!config)
        return;
    config_set_uint(config, "SimpleOutput", "VBitrate", newVBitrate);
    config_set_uint(config, "AdvOut", "FFVBitrate", newVBitrate);
    config_save(config);
    ...
    if (vencoder) {
        ...
        obs_data_set_int(vencoder_settings, "bitrate", newVBitrate);
        obs_data_set_int(vencoder_settings, "max_bitrate", newVBitrate);
...
void OBSBasic::ApplyLdeABitrateComboboxChange(int newABitrate)
{
    // NOTE: Since a QComboBox fires currentIndexChanged only on an actual change, we don't compare the old and new aBitrates here.

    auto *config = obs_frontend_get_profile_config();
    if (!config)
        return;
    config_set_uint(config, "SimpleOutput", "ABitrate", newABitrate);
    config_set_uint(config, "AdvOut", "FFABitrate", newABitrate);
    config_set_uint(config, "AdvOut", "Track1Bitrate", newABitrate);
    config_set_uint(config, "AdvOut", "Track2Bitrate", newABitrate);
    config_set_uint(config, "AdvOut", "Track3Bitrate", newABitrate);
    config_set_uint(config, "AdvOut", "Track4Bitrate", newABitrate);
    config_set_uint(config, "AdvOut", "Track5Bitrate", newABitrate);
    config_set_uint(config, "AdvOut", "Track6Bitrate", newABitrate);
    config_save(config);
    auto *output = obs_get_output_by_name("simple_stream");
    if (!output)
        output = obs_get_output_by_name("adv_stream");
    if (output) {
        ...
        obs_data_set_int(aencoder_settings, "bitrate", newABitrate);
        for (;;) {
            auto *aencoder =
...
void OBSBasic::InitLdeBitrateDock()
{
    auto videoCombo = ui->ldeBitrateDock_VideoCombo;
    auto audioCombo = ui->ldeBitrateDock_AudioCombo;

    videoCombo->addItem(QStringLiteral("1.2 Mbps ") + QTStr("Basic.Main.LdeBitrate.Video.Low"), QVariant::fromValue(1200));
    videoCombo->addItem(QStringLiteral("2.5 Mbps ") + QTStr("Basic.Main.LdeBitrate.Video.Balanced"), QVariant::fromValue(2500));
    videoCombo->addItem(QStringLiteral("4 Mbps ") + QTStr("Basic.Main.LdeBitrate.Video.High"), QVariant::fromValue(4000));
    videoCombo->addItem(QStringLiteral("6 Mbps ") + QTStr("Basic.Main.LdeBitrate.Video.VeryHigh"), QVariant::fromValue(6000));
    videoCombo->addItem(QStringLiteral("8 Mbps ") + QTStr("Basic.Main.LdeBitrate.Video.SuperHigh"), QVariant::fromValue(8000));
    videoCombo->addItem(QStringLiteral("12 Mbps ") + QTStr("Basic.Main.LdeBitrate.Video.UltraHigh"), QVariant::fromValue(12000));
    videoCombo->addItem(QStringLiteral("15 Mbps ") + QTStr("Basic.Main.LdeBitrate.Video.Extreme"), QVariant::fromValue(15000));
    videoCombo->addItem(QStringLiteral("20 Mbps ") + QTStr("Basic.Main.LdeBitrate.Video.AbsMax"), QVariant::fromValue(20000));

    audioCombo->addItem(QStringLiteral("160k ") + QTStr("Basic.Main.LdeBitrate.Audio.Default"), QVariant::fromValue(160));
    audioCombo->addItem(QStringLiteral("320k ") + QTStr("Basic.Main.LdeBitrate.Audio.High"), QVariant::fromValue(320));
    audioCombo->addItem(QStringLiteral("510k ") + QTStr("Basic.Main.LdeBitrate.Audio.Extreme"), QVariant::fromValue(510));

    UpdateLdeBitrateDockValues(/*force*/ true);

    connect(videoCombo, &QComboBox::currentIndexChanged, [=](int newIndex) {
        ldeChangingVBitrateCombo = true;
        auto newVBitrate = videoCombo->itemData(newIndex).toInt();
        ApplyLdeVBitrateComboboxChange(newVBitrate);
        ldeChangingVBitrateCombo = false;
    });

    connect(audioCombo, &QComboBox::currentIndexChanged, [=](int newIndex) {
        ldeChangingABitrateCombo = true;
        auto newABitrate = audioCombo->itemData(newIndex).toInt();
        ApplyLdeABitrateComboboxChange(newABitrate);
        ldeChangingABitrateCombo = false;
    });

    auto timer = new QTimer(this);
    connect(timer, &QTimer::timeout, this, [=]() { UpdateLdeBitrateDockValues(/*force*/ false); });
    timer->start(1000);
}
...
void OBSBasic::OBSInit()
{
	...
	ret = snprintf(fileName, sizeof(fileName),
		       "Louper/LDE/basic/scenes/%s.json", sceneCollection);
	if (ret <= 0)
	...
	if (obs_audio_monitoring_available()) {
		...
		int idx = ui->monitoringDevice->findData(QT_UTF8(device_id));
		if (idx != -1) {
			ui->monitoringDevice->setCurrentIndex(idx);
		} else {
			ui->monitoringDevice->insertItem(0, device_name,
							 device_id);
			QStandardItemModel *model =
				dynamic_cast<QStandardItemModel *>(
					ui->monitoringDevice->model());
			if (model) {
...
#ifdef __APPLE__
    DisableMacOSViewFullScreenMainMenuItem();
#endif
...
void OBSBasic::CreatePropertiesWindow(obs_source_t *source, bool deleteSourceOnCancel, obs_sceneitem_t *deleteSourceSceneitem)
{
	bool closed = true;
	if (properties)
	...
	properties = new OBSBasicProperties(this, source, /*deleteSourceOnCancel_*/ deleteSourceOnCancel, /*deleteSourceSceneitem_*/ deleteSourceSceneitem);
	properties->Init();
...
void OBSBasic::UpdateContextBar(bool force)
{
	...
	if (item) {
        ...
        auto showingMediaControls = false;

        if ((contextBarSize >= ContextBarSize_Minimized) &&
            (updateNeeded || force)) {
            ...
            ui->contextSourceLabel->show();
            ui->sourcePrePropertiesLine->show();
            ui->sourcePostFiltersLine->show();
        }
        ...
        if ((contextBarSize >= ContextBarSize_Minimized) &&
            ...
            if (flags & OBS_SOURCE_CONTROLLABLE_MEDIA) {
                if (!is_network_media_source(source, id)) {
                    ...
                    mediaControls->SetTimeVisible(false);
		...
		if ((contextBarSize >= ContextBarSize_Reduced) &&
            ...
            if (strcmp(id, "browser_source") == 0) {
				...
				ui->emptySpace->layout()->addWidget(c);
                   ...
                   strcmp(id, "louper_virtual_audio_2ch_input_capture") == 0 ||
        ...
        if (showingMediaControls) {
            ...
            ui->contextSourceLabel->hide();
            ui->sourcePrePropertiesLine->hide();
            ui->sourcePostFiltersLine->hide();
        } else {
            ui->contextSourceIconSpacer->hide();
        }
...
void OBSBasic::VolControlContextMenu()
{
	...
	// Hide the "VerticalLayout" option
	toggleControlLayoutAction.setVisible(false);
...
#if 0
#ifndef __APPLE__
	if (!LoadUpdateSettings()) {
		//first start
		WriteUpdateSettingsFile();
	}

	if (!skip_update_once && !skip_version && auto_update_enabled) {
		bool check_for_update = VerifyVersion(Latest_LDE_version);
...
bool OBSBasic::VerifyVersion(std::string input_version)
{
	bool check_for_update = true;
	int i_Latest_version[3] = {0, 0, 0};

	int array_step = 0;
	for (int i = 0; i < static_cast<int>(input_version.size()); i++) {
		if (input_version[i] == '.')
			array_step++;
		else if (input_version[i] < '0' || input_version[i] > '9') {
			check_for_update = false;
			break;
		} else {
			if (i_Latest_version[array_step] != 0)
				i_Latest_version[array_step] =
					i_Latest_version[array_step] * 10 +
					(input_version[i] - '0');
			else
				i_Latest_version[array_step] +=
					input_version[i] - '0';
	...
	if (check_for_update) {
		if (i_Latest_version[0] > LDE_version_major)
			check_for_update = true;
		else if (i_Latest_version[0] == LDE_version_major &&
			 i_Latest_version[1] > LDE_version_minor) {
			check_for_update = true;
		} else if (i_Latest_version[0] == LDE_version_major &&
			   i_Latest_version[1] == LDE_version_minor &&
			   i_Latest_version[2] > LDE_version_patch) {
			check_for_update = true;
		} else {
			check_for_update = false;
		}
...
void OBSBasic::CheckForUpdates(bool manualUpdate)
{
	...
	updateWindow = new OBSUpdate(this, manualUpdate,
				     QString::fromStdString(readBuffer),
				     &auto_update_enabled, &skip_update_once,
				     &skip_version, &update_settings_changed);
	updateWindow->show();

	updateWindow->setAttribute(Qt::WA_DeleteOnClose, true);
#endif
}
...
obs_service_t *OBSBasic::GetService()
{
	if (!service) {
		service =
			obs_service_create("rtmp_custom", NULL, NULL, nullptr);
		obs_service_release(service);
	}
	return service;
}
...
void OBSBasic::closeEvent(QCloseEvent *event)
{
	if (config_get_bool(GetGlobalConfig(), "General",
			    "EnableAutoUpdates") != auto_update_enabled) {
		auto_update_enabled = config_get_bool(
			GetGlobalConfig(), "General", "EnableAutoUpdates");
		update_settings_changed = true;
	}
...
void OBSBasic::on_action_Settings_triggered()
{
    ...
    if (outputHandler->StreamingActive()) {
        OBSMessageBox::information(this,
            QTStr("Basic.Main.LdeSettingsWhileStreamingAlert.Title"),
            QTStr("Basic.Main.LdeSettingsWhileStreamingAlert.Text"));
        return;
    }
	...
	{
        auto initialPageIndex = ldeNextShowSettingsPageIndex;
        ldeNextShowSettingsPageIndex = 0;

		OBSBasicSettings settings(this, initialPageIndex);
		settings.exec();
	}
...
void OBSBasic::CreateSourcePopupMenu(int idx, bool preview)
{
	...
	popup.addAction(QTStr("Fit to screen"), this,
			&OBSBasic::on_actionFitToScreen_triggered);

	popup.addAction(QTStr("Centre to screen"), this,
			&OBSBasic::on_actionCenterToScreen_triggered);
	...
	if (idx >= 0) {
        ...
        auto propertiesEnabled = obs_source_configurable(source);
#ifdef __APPLE__
        auto sourceId = obs_source_get_id(source);
        if ((!propertiesEnabled) && (Is_SourceId_LdeMacScreenCapture(sourceId) || Is_SourceId_LdeMacDesktopAudio(sourceId))) {
            propertiesEnabled = true;
        }
#endif
        action->setEnabled(propertiesEnabled);
	}
    ...
    if (preview) {
        ...
        QAction *previewWindow =
            popup.addAction(QTStr("Windowed Projector"), this,
	...
	//		actionHideMixer->setVisible(false);
	//		popup.addSeparator();
	//	}
...
// NOTE: This is a hackish way to get the scene item from AddSourceWithoutPopUp().
static obs_sceneitem_t *sAddSourceWithoutPopUp_Sceneitem = {};

static void AddSourceWithoutPopUp(void *data, obs_scene_t *scene)
{
	obs_sceneitem_t *sceneitem = obs_scene_add(scene, (obs_source_t *)data);
	obs_sceneitem_set_visible(sceneitem, true);
    sAddSourceWithoutPopUp_Sceneitem = sceneitem;
}

#ifdef __APPLE__
auto OBSBasic::Is_SourceId_LdeMacScreenCapture(const char *sourceId) -> bool
{
    return strcmp(sourceId, "screen_capture") == 0;
}

auto OBSBasic::Is_SourceId_LdeMacDesktopAudio(const char *sourceId) -> bool
{
    return strcmp(sourceId, "sck_audio_capture") == 0;
}

auto OBSBasic::Check_LdeMacScreenCaptureKitSource_PermissionsIfNeeded(const char *sourceId) -> bool /*allowedOrNa*/
{
    auto screenCapture = Is_SourceId_LdeMacScreenCapture(sourceId);
    auto desktopAudio = Is_SourceId_LdeMacDesktopAudio(sourceId);

    if (!(screenCapture || desktopAudio)) {
        return /*allowedOrNa*/ true;
    }

    MacPermissionStatus permissionStatus = CheckPermission(kScreenCapture);

    switch (permissionStatus) {
        case kPermissionNotDetermined:
        case kPermissionAuthorized: {
            return /*allowedOrNa*/ true;
            break;
        }
        case kPermissionRestricted:
        case kPermissionDenied: {
            const char* textKey = "";
            if (screenCapture) {
                textKey = "Basic.Main.LdeMacScreenCaptureKit.PermissionDenied.Text.ScreenCapture";
            } else if (desktopAudio) {
                textKey = "Basic.Main.LdeMacScreenCaptureKit.PermissionDenied.Text.DesktopAudio";
            }

            QMessageBox mb(QMessageBox::Critical,
                           QTStr("Basic.Main.LdeMacScreenCaptureKit.PermissionDenied.Title"),
                           QTStr(textKey));
            QPushButton *settingsButton = mb.addButton(QTStr("Basic.Main.LdeMacScreenCaptureKit.PermissionDenied.OpenSystemSettings"), QMessageBox::ActionRole);
            mb.addButton(QTStr("Basic.Main.LdeMacScreenCaptureKit.PermissionDenied.Later"), QMessageBox::RejectRole);
            mb.setDefaultButton(settingsButton);
            mb.exec();
            auto showSettings = (settingsButton == mb.clickedButton());

            if (showSettings) {
                OpenMacOSPrivacyPreferences("ScreenCapture");
            }

            return /*allowedOrNa*/ false;
            break;
        }
    }
}
...
void OBSBasic::AddSource(const char *id)
{
	if (id && *id) {
		...
		QString placeHolderText =
			QT_UTF8(obs_source_get_display_name(v_id));
...
#ifdef __APPLE__
        auto allowedOrNa = Check_LdeMacScreenCaptureKitSource_PermissionsIfNeeded(id);
        if (!allowedOrNa) {
            return;
        }
#endif

		source = obs_source_create(v_id, QT_TO_UTF8(text), NULL,
					   nullptr);
		obs_enter_graphics();
		obs_scene_atomic_update(scene, AddSourceWithoutPopUp, source);
        obs_sceneitem_t *sceneitem = sAddSourceWithoutPopUp_Sceneitem;
        sAddSourceWithoutPopUp_Sceneitem = {};
		obs_leave_graphics();
		obs_source_release(source);

        bool isGroup = (strcmp(id, "group") == 0);
        bool isConfigurable = obs_source_configurable(source);

		if ((!isGroup) && isConfigurable) {
			CreatePropertiesWindow(source, /*deleteSourceOnCancel*/ true, /*deleteSourceSceneitem*/ sceneitem);
		}
	}
}
...
QMenu *OBSBasic::CreateAddSourcePopupMenu()
{
	...
	auto addSource = [this](QMenu *popup, const char *type) {
		QString qname = QT_UTF8(obs_source_get_display_name(type));
		if (qname.isEmpty())
			return;
	...
	addSource(popup, "ndi_source");
	popup->addSeparator();

	addSource(popup, "screen_capture");
#ifdef _WIN32
	addSource(popup, "monitor_capture");
	addSource(popup, "display_capture");
	addSource(popup, "window_capture");
#endif
	addSource(popup, "xshm_input");

#ifdef __APPLE__
    if (__builtin_available(macOS 13.0, *)) {
        addSource(popup, "sck_audio_capture");
    }
    addSource(popup, "coreaudio_input_capture");
#endif
#ifdef __APPLE__
    if (obs_source_get_display_name("louper_virtual_audio_2ch_input_capture")) {
        addSource(popup, "louper_virtual_audio_2ch_input_capture");
    }
#endif
#ifdef _WIN32
	addSource(popup, "wasapi_input_capture");
	addSource(popup, "wasapi_output_capture");
	addSource(popup, "audio_capture");
#endif
	//addSource(popup, "pulse_input_capture");
	//addSource(popup, "alsa_input_capture");

    popup->addSeparator();

    addSource(popup, "decklink-input");
    addSource(popup, "aja-source");
    addSource(popup, "dshow_input");
    addSource(popup, "av_capture_input");

	popup->addSeparator();

	addSource(popup, "ffmpeg_source");

    popup->addSeparator();

    // Create a submenu for additional sources
    QMenu *moreSubMenu = new QMenu(QTStr("More"), popup);

    // Add specific sources to the "More" submenu
    addSource(moreSubMenu, "color_source");
    addSource(moreSubMenu, "image_source");
#ifndef _WIN32
    addSource(moreSubMenu, "text_ft2_source");
#endif
    addSource(moreSubMenu, "text_gdiplus");
#ifdef __APPLE__
    if (__builtin_available(macOS 13.0, *)) {
    } else {
        addSource(moreSubMenu, "window_capture");
        addSource(moreSubMenu, "display_capture");
    }
#endif

    // Add the "More" submenu to the main popup menu
    popup->addMenu(moreSubMenu);
...
//
//    stringstream ss;
//    ss << "LDE " << App()->GetVersionString(false)
...
void OBSBasic::on_actionLdeUploadLogs_triggered()
{
    if (!_ldeLogUploader)
        _ldeLogUploader = new LdeLogUploader();

    _ldeLogUploader->show();
    _ldeLogUploader->setWindowState(
        (_ldeLogUploader->windowState() & ~Qt::WindowMinimized) |
        Qt::WindowActive);
    _ldeLogUploader->activateWindow();
    _ldeLogUploader->raise();
}

void OBSBasic::on_actionShowLogs_triggered()
{
	char logDir[512];
	if (GetConfigPath(logDir, sizeof(logDir), "Louper/LDE/logs") <= 0)
		return;
...
//{
//    //UploadLog("Louper/LDE/logs", App()->GetCurrentLog(), false);
//}
//
//void OBSBasic::on_actionUploadLastLog_triggered()
//{
//    //UploadLog("Louper/LDE/logs", App()->GetLastLog(), false);
//}
...
{
	char logDir[512];
	if (GetConfigPath(logDir, sizeof(logDir), "Louper/LDE/crashes") <= 0)
		return;
...
{
	//UploadLog("Louper/LDE/crashes", App()->GetLastCrashLog(), true);
}
...
void OBSBasic::on_actionResetSettings_triggered()
{
    auto parent = this;
    auto title = QTStr("Lde.ResetSettingsAlert.Title");
    auto text = QTStr("Lde.ResetSettingsAlert.Text");
    ...
    {
        QPushButton *button = mb.addButton(QMessageBox::Cancel);
        button->setText(QTStr("Cancel"));
    }
    {
        mb.addButton(QTStr("Lde.ResetSettingsAlert.ResetButton"), QMessageBox::DestructiveRole);
    }

    auto mbResult = (QMessageBox::StandardButton)mb.exec();
    if (mbResult != QMessageBox::Cancel) {
        restart = false;
        restart_reset_settings = true;
        close();
    }
}
...
void OBSBasic::StartStreaming()
{
...

    {
        ...
        auto protocolCStringQ = obs_service_get_protocol(service);
        if (protocolCStringQ) {
            const char *serverCStringQ =
                obs_data_get_string(settings, "server");
            if ((!serverCStringQ || !*serverCStringQ) &&
                ((strcmp("RTMP", protocolCStringQ) == 0) ||
                 (strcmp("SRT", protocolCStringQ) == 0))) {
                serverCStringQ = obs_data_get_string(settings, "key");
            }
            if (serverCStringQ && *serverCStringQ) {
                if (!LdeIsStreamServerUrlAcceptable(serverCStringQ)) {
    ...
    App()->SendMixpanelEvent(MixpanelEventKind::StreamStarted, GetService(), ui->sources->GetStm());
}
...
void OBSBasic::ReconnectGuard_Reset()
{
    reconnectGuard_lastReconnectSuccessUnixTimeQz = 0;
    reconnectGuard_extraDisconnectCount = 0;
}

void OBSBasic::ShowTroubleshootStreamKeyAlert()
{
    QMessageBox mb(QMessageBox::Critical,
                   QTStr("Output.ConnectFail.Title"),
                   QTStr("LdeTroubleshootStreamKeyAlert.Message"));
    auto troubleshootButton = mb.addButton(QTStr("LdeTroubleshootStreamKeyAlert.Troubleshoot"), QMessageBox::ActionRole);
    mb.addButton(QTStr("LdeTroubleshootStreamKeyAlert.Dismiss"), QMessageBox::RejectRole);
    mb.setDefaultButton(troubleshootButton);
    mb.exec();
    auto troubleshootPressed = (troubleshootButton == mb.clickedButton());

    if (troubleshootPressed) {
        QDesktopServices::openUrl(QUrl("https://docs.louper.io/troubleshooting/stream-key-troubleshooting"));
    }
}

void OBSBasic::ReconnectGuard_HandleDeactivate()
{
    int64_t now = time(nullptr);
    int64_t deltaT = abs(now - reconnectGuard_lastReconnectSuccessUnixTimeQz);
    if (deltaT < 15) {
        // NOTE: We treat this as a potential ReconnectGuard case.
        reconnectGuard_extraDisconnectCount += 1;
        if ((1 + reconnectGuard_extraDisconnectCount) >= 2) {
            StopStreaming();
            ShowTroubleshootStreamKeyAlert();
        }
    } else {
        // NOTE: We treat this as an unrelated disconnect.
        ReconnectGuard_Reset();
    }
}

void OBSBasic::ReconnectGuard_HandleReconnectSuccess()
{
    reconnectGuard_lastReconnectSuccessUnixTimeQz = time(nullptr);
}

void OBSBasic::BgOBSOutputDeactivate(void *data, calldata_t */*params*/)
{
    auto object = reinterpret_cast<OBSBasic*>(data);
    QMetaObject::invokeMethod(object, "ReconnectGuard_HandleDeactivate");
}

void OBSBasic::BgOBSOutputReconnectSuccess(void *data, calldata_t */*params*/)
{
    auto object = reinterpret_cast<OBSBasic*>(data);
    QMetaObject::invokeMethod(object, "ReconnectGuard_HandleReconnectSuccess");
}
...
void OBSBasic::StopStreaming()
{
    ...
    App()->SendMixpanelEvent(MixpanelEventKind::StreamStopped, GetService(), /*sourceTreeModel*/ {});
}
...
void OBSBasic::StreamingStart()
{
    ...
    signal_handler_connect(obs_output_get_signal_handler(outputHandler->streamOutput), "deactivate", BgOBSOutputDeactivate, this);
    signal_handler_connect(obs_output_get_signal_handler(outputHandler->streamOutput), "reconnect_success", BgOBSOutputReconnectSuccess,this);
...
{
    signal_handler_disconnect(obs_output_get_signal_handler(outputHandler->streamOutput), "reconnect_success", BgOBSOutputReconnectSuccess, this);
    signal_handler_disconnect(obs_output_get_signal_handler(outputHandler->streamOutput), "deactivate", BgOBSOutputDeactivate, this);

	const char *errorDescription = "";
	DStr errorMessage;
	bool use_last_error = false;
	bool encode_error = false;
    bool showTroubleshootStreamKeyAlert = false;
	...
	case OBS_OUTPUT_INVALID_STREAM:
        ...
        showTroubleshootStreamKeyAlert = true;
		break;
    ...
    if (showTroubleshootStreamKeyAlert) {
        ShowTroubleshootStreamKeyAlert();
...
void OBSBasic::on_streamKeyButton_clicked()
{
    ldeNextShowSettingsPageIndex = 0;
    on_action_Settings_triggered();
}
...
{
    ldeNextShowSettingsPageIndex = 1;
	on_action_Settings_triggered();
}
...
{
	char path[512];
	int ret = GetConfigPath(path, 512, "Louper/LDE");
	if (ret <= 0)
		return;
...
void OBSBasic::on_previewDisabledWidget_customContextMenuRequested()
{
	...
	QAction *previewWindow = popup.addAction(QTStr("Windowed Projector"), this,
						 &OBSBasic::OpenPreviewWindow);
...
auto OBSBasic::HasLdeSourcePreviewProjector() -> bool
{
    for (auto currentProjector: projectors) {
        if (currentProjector->GetProjectorType() == ProjectorType::Preview) {
            return true;
        }
    }
    return false;
...
OBSProjector *OBSBasic::OpenProjector(obs_source_t *source, int monitor,
				      ProjectorType type)
{
    OBSProjector* existingProjectorQ{};
    //
    for (auto currentProjector: projectors) {
        if (currentProjector->GetProjectorType() == type) {
            existingProjectorQ = currentProjector;
            break;
        }
    }

    if (!existingProjectorQ) {
        OBSProjector *projector = new OBSProjector(nullptr, source, monitor, type);
        projectors.emplace_back(projector);
        update_LdeSourcePreviewButton_BgColor();
        return projector;
    } else {
        DeleteProjector(existingProjectorQ);
        return {};
    }
...
int OBSBasic::GetProfilePath(char *path, size_t size, const char *file) const
{
	...
	ret = GetConfigPath(profiles_path, 512, "Louper/LDE/basic/profiles");
	if (ret <= 0)
		return ret;
...
#undef RESET_DOCKLIST
	...
	ui->ldeBitrateDock->setVisible(true);
...
void OBSBasic::on_lockDocks_toggled(bool lock)
{
    ...
    //ui->scenesDock->setMinimumHeight(160);
    //ui->scenesDock->setMaximumHeight(161);
	ui->sourcesDock->setFeatures(mainFeatures);
    //ui->sourcesDock->setMinimumHeight(160);
    //ui->sourcesDock->setMaximumHeight(161);
	ui->mixerDock->setFeatures(mainFeatures);
    //ui->mixerDock->setMaximumHeight(200);
    //ui->mixerDock->setMinimumHeight(200);
	ui->transitionsDock->setFeatures(mainFeatures);
	ui->ldeBitrateDock->setFeatures(mainFeatures);
    ui->ldeBitrateDock->setMinimumHeight(62);
	ui->ldeBitrateDock->setMaximumHeight(62);
	ui->ldeBitrateDock->setTitleBarWidget(new QWidget());
	ui->controlsDock->setFeatures(mainFeatures);
    ui->controlsDock->setMinimumHeight(76);
	ui->controlsDock->setMaximumHeight(76);
	ui->controlsDock->setTitleBarWidget(new QWidget());
...
void OBSBasic::on_actionShowLdeAuthors_triggered()
{
    if (_ldeAuthors)
        _ldeAuthors->close();
    
    _ldeAuthors = new LdeAuthors(this);
    _ldeAuthors->show();
    
    _ldeAuthors->setAttribute(Qt::WA_DeleteOnClose, true);
}

void OBSBasic::on_actionShowLdeLicense_triggered()
{
    if (_ldeLicense)
        _ldeLicense->close();
    
    _ldeLicense = new LdeLicense(this);
    _ldeLicense->show();
    
    _ldeLicense->setAttribute(Qt::WA_DeleteOnClose, true);
}
...
void OBSBasic::update_LdeSourcePreviewButton_BgColor()
{
    if (!HasLdeSourcePreviewProjector()) {
        QString newStylesheet{""};
        ui->ldeSourcePreviewButton->setStyleSheet(newStylesheet);
    } else {
        QString newStylesheet{"QPushButton{background-color: #606060;} QPushButton:hover{background-color: #808080;} QPushButton:pressed{background-color: #202020;}"};
        ui->ldeSourcePreviewButton->setStyleSheet(newStylesheet);
    }
}

void OBSBasic::on_ldeSourcePreviewButton_clicked()
{
    OpenProjector(nullptr, -1, ProjectorType::Preview);
}
...
void OBSBasic::FillAudioMonitoringDevices()
{
...
std::string OBSBasic::GetLastLDEVersion()
{
	...
	start_number += key_version.size();

	if (start_number >= readBuffer.size())
		return new_version;

	for (; start_number < readBuffer.size(); start_number++) {
		if (readBuffer[start_number] == ' ')
			break;
		new_version += readBuffer[start_number];
	}

	return new_version;
}

bool OBSBasic::WriteUpdateSettingsFile()
{
	std::string filePath;

	if (skip_version)
		version_to_skip = Latest_LDE_version;

#ifdef _WIN32
	std::string appdata = getenv("APPDATA");

	filePath = appdata + "\\Louper\\LDE\\update_settings.json";
#else
	std::string userhome = getenv("HOME");
	filePath = userhome +
		   "/Library/Application Support/Louper/LDE/update_settings.json";
#endif

	std::string output_file = "{";

	output_file += "\"autoUpdateEnable\":";
	output_file += auto_update_enabled ? "1," : "0,";

	output_file += "\"skipUpdateOnce\":";
	output_file += skip_update_once ? "1," : "0,";

	output_file += "\"skipVersion\":";
	output_file += skip_version ? "1," : "0,";

	output_file += "\"versionToSkip\":";
	output_file += "\"" + version_to_skip + "\"";
...
OBSBasic::ReadUpdateSettingsFile(std::string filePath)
{
...
std::string
OBSBasic::FindUpdateSettingsValue(std::map<std::string, std::string> &values,
				  std::string key)
{
	std::string value;
...
bool OBSBasic::LoadUpdateSettings()
{
	...
	values =
		ReadUpdateSettingsFile(appdata + "\\Louper\\LDE\\update_settings.json");
#else
	std::string userhome = getenv("HOME");
	values = ReadUpdateSettingsFile(
		userhome +
		"/Library/Application Support/Louper/LDE/update_settings.json");
#endif

	std::string new_version = GetLastLDEVersion();

	if (new_version.empty())
		Latest_LDE_version = LDEGetVersion();
	else
		Latest_LDE_version = new_version;

	if (values.empty())
		return false;

	bool temp_auto_update_enabled =
		std::stoi(FindUpdateSettingsValue(values, "autoUpdateEnable"));
	bool temp_skip_update_once =
		std::stoi(FindUpdateSettingsValue(values, "skipUpdateOnce"));
	bool temp_skip_version =
		std::stoi(FindUpdateSettingsValue(values, "skipVersion"));
	std::string temp_version_to_skip =
		FindUpdateSettingsValue(values, "versionToSkip");

	if (auto_update_enabled != temp_auto_update_enabled ||
	    skip_update_once != temp_skip_update_once ||
	    skip_version != temp_skip_version ||
	    version_to_skip != temp_version_to_skip) {
		auto_update_enabled = temp_auto_update_enabled;
		skip_update_once = temp_skip_update_once;
		skip_version = temp_skip_version;
		version_to_skip = temp_version_to_skip;
		...
		if (skip_version == true && check_for_update &&
		    version_to_skip == Latest_LDE_version) {
			skip_version = true;
		} else {
			skip_version = false;
...
// NOTE: This is lifted from plugins/obs-outpus/parseurl.c and simplified for our needs.
static auto Simplified_RTMP_ParseURL(const char *url) -> std::pair<const char* /*hostnameBytesQ*/, size_t /*hostnameByteCount*/> /*hostnameQPair*/
{
    const char* p{};
    ...
    auto v6    = strchr(p, ']');
    auto slash = strchr(p, '/');
    auto col   = strchr((v6 && v6 < slash) ? v6 : p, ':');

    {
        ...
        if(hostlen < 256)
        {
            return /*hostnameQPair*/ std::pair<const char* /*hostnameBytesQ*/, size_t /*hostnameByteCount*/>(p, hostlen);
        }
        else
        {
            // NOTE: Hostname exceeds 255 characters.
            return /*hostnameQPair*/ {};
        }
    }
}
...
auto LdeIsStreamServerUrlAcceptable(const char* serverUrl) -> bool
{
    ...
    auto hostnameQPair = Simplified_RTMP_ParseURL(serverUrl);
    //
    if (!hostnameQPair.first) {
        return false;
    }

    auto hostnameStdStringView = std::string_view{hostnameQPair.first, hostnameQPair.second};

#define iLOUPER_HOSTNAME "louper.io"

    if (hostnameStdStringView == std::string_view{iLOUPER_HOSTNAME}) {
        return true;
    }

    {
        auto suffix = std::string_view{"." iLOUPER_HOSTNAME};

        auto hostnameStdStringViewByteCount = hostnameStdStringView.size();
        auto suffixByteCount = suffix.size();

        if (hostnameStdStringViewByteCount < suffixByteCount) {
            return false;
        }
        return hostnameStdStringView.substr(hostnameStdStringViewByteCount - suffixByteCount) == suffix;
    }

#undef iLOUPER_HOSTNAME
}

auto LdeIsStreamServerUrlAcceptable(const QString& serverUrl) -> bool
{
    auto serverUrlStdString = serverUrl.toStdString();
    return LdeIsStreamServerUrlAcceptable(serverUrlStdString.c_str());
}
...
auto LdeWarnServerUrlUnacceptable(QWidget* parent) -> void
{
    QMessageBox::critical(
        parent,
        QTStr("LdeServerUrlUnacceptableAlert.Title"),
        QTStr("LdeServerUrlUnacceptableAlert.Message"),
