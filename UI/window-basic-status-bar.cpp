...
static auto FixFrameMargins(QFrame* frame) -> void
{
    const int kHmargin = 5;
    const int kLeftMargin = 1 + kHmargin;
    const int kRightMargin = kHmargin;

    auto initialContentsMargins = frame->contentsMargins();

    auto adjustedContentsMargins = initialContentsMargins;
    adjustedContentsMargins.setLeft(kLeftMargin);
    adjustedContentsMargins.setRight(kRightMargin);

    if (initialContentsMargins == adjustedContentsMargins) {
        return;
    }

    frame->setContentsMargins(adjustedContentsMargins);
}

static auto UpdateLabelMinWidth(QLabel* label) -> void
{
					...
					.pixmap(QSize(16, 16)))
{
    ...
    statusWidget->ui->cpuFrame->setVisible(false);
    ...
    FixFrameMargins(statusWidget->ui->issuesFrame);
    FixFrameMargins(statusWidget->ui->networkFrame);
    FixFrameMargins(statusWidget->ui->streamFrame);
    FixFrameMargins(statusWidget->ui->fpsFrame);
}
...
void OBSBasicStatusBar::Deactivate()
{
	...
	if (main->outputHandler && !main->outputHandler->Active()) {
        ...
        UpdateLabelMinWidth(statusWidget->ui->kbps);
...
void OBSBasicStatusBar::UpdateCPUUsage()
{
    ...
    UpdateLabelMinWidth(statusWidget->ui->cpuUsage);
...
void OBSBasicStatusBar::UpdateCurrentFPS()
{
	...
	QString text = QString::asprintf("%.2f FPS", targetFPS);

	statusWidget->ui->fpsCurrent->setText(text);
    UpdateLabelMinWidth(statusWidget->ui->fpsCurrent);
}
