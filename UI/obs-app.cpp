...
#if 0 // defined(_WIN32) || defined(ENABLE_SPARKLE_UPDATER)
...
#define kDefaultProfileName "Profile 1"
...
bool opt_lde_reset_settings = false;

bool restart = false;
bool restart_reset_settings = false;
...
bool OBSApp::InitGlobalConfigDefaults()
{
    ...
    config_set_default_bool(globalConfig, "Audio", "LdeAutoVolumeReduction", true);
...
static bool MakeUserDirs()
{
	...
	if (GetConfigPath(path, sizeof(path), "Louper/LDE/basic") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

	if (GetConfigPath(path, sizeof(path), "Louper/LDE/logs") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

	if (GetConfigPath(path, sizeof(path), "Louper/LDE/profiler_data") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

#ifdef _WIN32
	if (GetConfigPath(path, sizeof(path), "Louper/LDE/crashes") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;
#endif
	...
	if (GetConfigPath(path, sizeof(path), "Louper/LDE/updates") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;
#endif

	if (GetConfigPath(path, sizeof(path), "Louper/LDE/plugin_config") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;
...
static bool MakeUserProfileDirs()
{
	...
	if (GetConfigPath(path, sizeof(path), "Louper/LDE/basic/profiles") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

	if (GetConfigPath(path, sizeof(path), "Louper/LDE/basic/scenes") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;
...
bool OBSApp::InitGlobalConfig()
{
	...
	int len = GetConfigPath(path, sizeof(path), "Louper/LDE/global.ini");
	if (len <= 0) {
		return false;
	}
    ...
    if (!config_has_user_value(globalConfig, "General", "LocalId")) {
        config_set_string(globalConfig, "General", "LocalId", GenId().c_str());
    }
...
bool OBSApp::InitTheme()
{
	...
	char userDir[512];
	if (GetConfigPath(userDir, sizeof(userDir), "Louper/LDE/themes")) {
...
bool LoadBranchesFile(vector<UpdateBranch> &out)
{
	...
	BPtr<char> branchesFilePath =
		GetConfigPathPtr("Louper/LDE/updates/branches.json");
	  ...
	  profilerNameStore(store)
{
    ...
    {
        QScreen* screenQ{};
        const char* geometryCStringQ = config_get_string(GlobalConfig(), "BasicWindow", "geometry");
        if (geometryCStringQ) {
            auto geometryByteArray = QByteArray::fromBase64(QByteArray(geometryCStringQ));
            QWidget dummyWidget{};
            dummyWidget.restoreGeometry(geometryByteArray);
            screenQ = QGuiApplication::screenAt(dummyWidget.frameGeometry().center());
        }
        if (!screenQ) {
            screenQ = QGuiApplication::primaryScreen();
        }

        auto use2x = false;
#ifdef __APPLE__
        if (screenQ->devicePixelRatio() > 1.5) {
            use2x = true;
        }
#endif

        if (use2x) {
            QPixmap splashPixmap(":res/images/LdeSplash@2x.png");
            _ldeSplash = new LdeSplashScreen(screenQ, splashPixmap);
        } else {
            QPixmap splashPixmap(":res/images/LdeSplash.png");
            _ldeSplash = new LdeSplashScreen(screenQ, splashPixmap);
        }
        _ldeSplash->setWindowFlags(_ldeSplash->windowFlags() | Qt::WindowStaysOnTopHint);
#ifdef __APPLE__
        FixLdeSplashScreen(_ldeSplash);
#endif
...
static void move_basic_to_profiles(void)
{
	...
	if (GetConfigPath(path, 512, "Louper/LDE/basic") <= 0)
		return;
	if (!os_file_exists(path))
		return;

	/* if the profiles directory doesn't already exist */
	if (GetConfigPath(new_path, 512, "Louper/LDE/basic/profiles") <= 0)
		return;
	...
	strcat(new_path, "/");
	strcat(new_path, kDefaultProfileName);
	if (os_mkdir(new_path) == MKDIR_ERROR)
		return;
...
static void move_basic_to_scene_collections(void)
{
	...
	if (GetConfigPath(new_path, 512, "Louper/LDE/basic/scenes") <= 0)
		return;
	if (os_file_exists(new_path))
		return;
...
void OBSApp::AppInit()
{
    ...
    // NOTE: Moved to constructor, as config geometry is used there to properly position the splash screen.
    //if (!MakeUserDirs())
    //    throw "Failed to create required user directories";
    //if (!InitGlobalConfig())
    //    throw "Failed to initialize global config";
...
static void create_log_file(fstream &logFile)
{
	...
	get_last_log(false, "Louper/LDE/logs", lastLogFile);
#ifdef _WIN32
	get_last_log(true, "Louper/LDE/crashes", lastCrashLogFile);
#endif

	currentLogFile = GenerateTimeDateFilename("txt");
	dst << "Louper/LDE/logs/" << currentLogFile.c_str();
	...
	if (logFile.is_open()) {
		delete_oldest_file(false, "Louper/LDE/logs");
...
#define LITERAL_SIZE(x) x, (sizeof(x) - 1)
	ostringstream dst;
	dst.write(LITERAL_SIZE("Louper/LDE/profiler_data/"));
...
extern void send_application_launch();
...
static int run_program(fstream &logFile, int argc, char *argv[])
{
	...
	try {
		...
		program.AppInit();
		delete_oldest_file(false, "Louper/LDE/profiler_data");
    ...
    if (restart || restart_reset_settings) {
        arguments = qApp->arguments();

        if (restart_reset_settings) {
            arguments.append("--lde-reset-settings");
        } else {
            arguments.removeAll("--lde-reset-settings");
        }
    }
			       ...
			       void * /* param */)
{
	...
	string crashFilePath = "Louper/LDE/crashes";
...
bool GetUnusedSceneCollectionFile(std::string &name, std::string &file)
{
	...
	ret = GetConfigPath(path, sizeof(path), "Louper/LDE/basic/scenes/");
	if (ret <= 0) {
...
{
	char path[512];
	int pathlen = GetConfigPath(path, 512, "Louper/LDE/basic/profiles");
...
static void check_safe_mode_sentinel(void)
{
	...
	BPtr sentinelPath = GetConfigPathPtr("Louper/LDE/safe_mode");
...
static auto GenerateGUID(std::string &guid) -> void
{
...
static auto Extract_SixCharIdQe_FromRtmpLikeUrl(QUrl url) -> std::string
{
    auto path = url.path();
    auto kPrefix = QString{"/app/"};

    if (!path.startsWith(kPrefix)) {
        return std::string{};
    }

    path = path.remove(/*position*/ 0, /*n*/ kPrefix.length());
    return std::string{path.toUtf8().constData()};
}

static auto ParseStreamKeyForMixpanel(const char* streamKeyCString, std::string* domainStdStringQePtr, std::string* sixCharIdStdStringQePtr) -> void
{
    auto& domainStdStringQe = *domainStdStringQePtr;
    auto& sixCharIdStdStringQe = *sixCharIdStdStringQePtr;

    QUrl qUrl{streamKeyCString};
    domainStdStringQe = std::string{qUrl.host().toUtf8().constData()};

    if ((qUrl.scheme() == QString{"rtmp"}) || (qUrl.scheme() == QString{"rtmps"})) {
        sixCharIdStdStringQe = Extract_SixCharIdQe_FromRtmpLikeUrl(qUrl);
    } else if (qUrl.scheme() == QString{"srt"}) {
        auto query = qUrl.query(QUrl::FullyDecoded);
        auto kPrefix = QString{"streamid="};
        if (query.startsWith(kPrefix)) {
            auto subUrl = QUrl{query.remove(/*position*/ 0, /*n*/ kPrefix.length())};
            sixCharIdStdStringQe = Extract_SixCharIdQe_FromRtmpLikeUrl(subUrl);
        }
    }
}

auto OBSApp::SendMixpanelEvent(MixpanelEventKind eventKind, obs_service_t* service, SourceTreeModel* sourceTreeModel) -> void
{
    const char* kUrl = "https://api-js.mixpanel.com/track/?ip=1";

    auto globalConfig = GetGlobalConfig();
    auto profileConfig = obs_frontend_get_profile_config();

    std::string eventNameCore{};
    //
    switch (eventKind) {
        case MixpanelEventKind::AppStarted: { eventNameCore = "Application Launched"; break; }
        case MixpanelEventKind::StreamStarted: { eventNameCore = "Stream Started"; break; }
        case MixpanelEventKind::StreamStopped: { eventNameCore = "Stream Stopped"; break; }
    ...
    int64_t unixTimeInt = time(nullptr);
    auto distinctId = config_get_string(globalConfig, "General", "LocalId");
    //
    std::string insertId{};
    GenerateGUID(insertId);

    auto ldeVersion = obs_get_version_string();
    auto osVersion = QSysInfo::productVersion().toStdString();

    auto propertiesJson = nlohmann::json{
        {"$insert_id", insertId},
...
#error Not implemented
    ...
    switch (eventKind) {
        case MixpanelEventKind::AppStarted:
        case MixpanelEventKind::LogsUploaded: {
            break;
        }
        case MixpanelEventKind::StreamStarted:
        case MixpanelEventKind::StreamStopped: {
            const char* keyCStringQ{};
            const char* protocolCStringQ{};
            {
                OBSDataAutoRelease settings = obs_service_get_settings(service);

                protocolCStringQ = obs_service_get_protocol(service);
                if (protocolCStringQ) {
                    if ((strcmp("RTMP", protocolCStringQ) == 0) || (strcmp("SRT", protocolCStringQ) == 0)) {
                        keyCStringQ = obs_data_get_string(settings, "key");
                    }
                }
            }

            std::string domainStdStringQe{};
            std::string sixCharIdStdStringQe{};
            ParseStreamKeyForMixpanel(keyCStringQ, &domainStdStringQe, &sixCharIdStdStringQe);

            if (!sixCharIdStdStringQe.empty()) {
                propertiesJson += nlohmann::json::object_t::value_type("6-Char ID", sixCharIdStdStringQe);
            }
            if (!domainStdStringQe.empty()) {
                propertiesJson += nlohmann::json::object_t::value_type("Domain", domainStdStringQe);
            }

            if (protocolCStringQ) {
                propertiesJson += nlohmann::json::object_t::value_type("Protocol", protocolCStringQ);
            }

            auto videoBitrate = config_get_uint(profileConfig, "SimpleOutput", "VBitrate");
            propertiesJson += nlohmann::json::object_t::value_type("Bitrate Video", videoBitrate);
            //
            auto audioBitrate = config_get_uint(profileConfig, "SimpleOutput", "ABitrate");
            propertiesJson += nlohmann::json::object_t::value_type("Bitrate Audio", audioBitrate);
            //
            auto videoEncoder = config_get_string(profileConfig, "SimpleOutput", "StreamEncoder");
            propertiesJson += nlohmann::json::object_t::value_type("Video Encoder", videoEncoder);
            //
            auto audioMonitoringDevice = config_get_string(profileConfig, "Audio", "MonitoringDeviceName");
            propertiesJson += nlohmann::json::object_t::value_type("Audio Monitoring Device", audioMonitoringDevice);
            break;
        }
    }

    static int64_t streamStartedUnixTimeIntQz{};
    //
    switch (eventKind) {
        case MixpanelEventKind::AppStarted:
        case MixpanelEventKind::LogsUploaded: {
        ...
        case MixpanelEventKind::StreamStarted: {
            streamStartedUnixTimeIntQz = time(nullptr);
            break;
        }
        case MixpanelEventKind::StreamStopped: {
            int64_t now = time(nullptr);
            int64_t durationS = std::max(int64_t{0}, now - streamStartedUnixTimeIntQz);
            streamStartedUnixTimeIntQz = 0;
            //
            propertiesJson += nlohmann::json::object_t::value_type("Duration", durationS);
            break;
        }
    }
    ...
    switch (eventKind) {
        ...
        case MixpanelEventKind::StreamStarted: {
            auto sourcesJson = nlohmann::json::array({});
            //
            auto sourceCount = sourceTreeModel->LdeGetItemCount();
            for (int sourceIndex = 0; sourceIndex < sourceCount; sourceIndex += 1) {
                auto sceneItem = sourceTreeModel->LdeGetItemAtIndex(sourceIndex);
                obs_source_t* source = obs_sceneitem_get_source(sceneItem);
                auto sourceId = obs_source_get_id(source);
                auto sourceName = obs_source_get_display_name(sourceId);
                sourcesJson.push_back(sourceName);
            }

            propertiesJson["Sources"] = std::move(sourcesJson);
            break;
        }
    }
    ...
    auto dataStdString = dataJson.dump();
    //blog(LOG_DEBUG, "SendMixpanelEvent: dataStdString: %s", dataStdString.c_str());
    auto bodyStdString = std::string{"data="} + UrlEncode(dataStdString);

    std::thread([kUrl, headers{std::move(headers)}, bodyStdString{std::move(bodyStdString)}](){
        RunPostRequest(
            /*url*/ kUrl,
            /*headers*/ headers,
            /*body*/ bodyStdString.c_str(),
            /*timeoutSQz*/ 15);
    }).detach();
}

auto OBSApp::HideLdeSplash() -> void
{
    if (!_ldeSplash) {
        return;
    }
    _ldeSplash->finish(GetMainWindow());
    _ldeSplash = {};
}
...
int main(int argc, char *argv[])
{
	...
	for (int i = 1; i < argc; i++) {
        ...
        } else if (arg_is(argv[i], "--lde-reset-settings", nullptr)) {
            opt_lde_reset_settings = true;
    ...
    if (opt_lde_reset_settings) {
        char path[512];
        int pathlen = GetConfigPath(path, 512, "Louper/LDE");
	...
	if (restart /*|| restart_safe*/ || restart_reset_settings) {
