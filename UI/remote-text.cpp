...
void RemoteTextThread::run()
{
	...
	string versionString("User-Agent: lde ");
...
{
    const auto encodedCString = curl_easy_escape(nullptr, input.c_str(), static_cast<int>(input.length()));
    std::string encodedStdString{encodedCString};
    curl_free(encodedCString);
    return encodedStdString;
}
    ...
    -> void
{
    ...
    if (curl) {
        ...
        if (timeoutSQz > 0) {
            curl_easy_setopt(curl.get(), CURLOPT_TIMEOUT, timeoutSQz);
        }
        ...
        code = curl_easy_perform(curl.get());
        if (code != CURLE_OK) {
            blog(LOG_WARNING, "RunPostRequest: HTTP request failed. %s", strlen(error) ? error : curl_easy_strerror(code));
        } else {
            //blog(LOG_DEBUG, "RunPostRequest: HTTP request succeeded: %s", str.c_str());
        }
