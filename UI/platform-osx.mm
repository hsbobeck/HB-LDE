...
void DisableMacOSViewFullScreenMainMenuItem()
{
    [[NSUserDefaults standardUserDefaults] setBool:NO forKey:@"NSFullScreenMenuItemEverywhere"];
}

auto LdeIsLbsAppInstalled() -> bool
{
    NSString* bundleId = @"com.louper.lbs";
    if (@available(macOS 12.0, *)) {
        NSArray<NSURL*>* urls = [[NSWorkspace sharedWorkspace] URLsForApplicationsWithBundleIdentifier:bundleId];
        return urls.count > 0;
    } else {
        NSURL* urlQ = [[NSWorkspace sharedWorkspace] URLForApplicationWithBundleIdentifier:bundleId];
        return urlQ != nil;
    }
}

auto LdeShow_LbsAppInstalled_Alert() -> bool /*doNotWarnAgain*/
{
    NSAlert* alert = [[NSAlert alloc] init];
    alert.alertStyle = NSAlertStyleInformational;
    alert.messageText = @"LBS detected in Applications folder or elsewhere on your computer.";
    alert.informativeText = @"Since June 2024, LBS has been renamed to LDE, and the latest version of the application is called LDE.\n\nWe recommend removing LBS from your system.";
    alert.showsSuppressionButton = YES;
    alert.suppressionButton.title = @"Do not warn me again";
    [alert runModal];
    return /*doNotWarnAgain*/ (NSControlStateValueOn == alert.suppressionButton.state);
}

auto LdeGetScreenAvailableHeightQz(QWidget* /*window*/ qWindowQ) -> int
{
    if (!qWindowQ) {
        return {};
    }
    NSView* nativeViewQ = (__bridge NSView*)reinterpret_cast<void*>(qWindowQ->winId());
    if (!nativeViewQ) {
        return {};
    }
    NSWindow* nativeWindowQ = [nativeViewQ window];
    if (!nativeWindowQ) {
        return {};
    }
    NSScreen* nativeScreenQ = [nativeWindowQ screen];
    if (!nativeScreenQ) {
        return {};
    }
    return nativeScreenQ.visibleFrame.size.height;
}

auto FixLdeSplashScreen(QSplashScreen* splash) -> void
{
    NSView* nativeViewQ = (__bridge NSView*)reinterpret_cast<void*>(splash->winId());
    if (!nativeViewQ) {
        return;
    }
    NSWindow* nativeWindowQ = [nativeViewQ window];
