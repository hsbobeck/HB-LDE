				   ...
				   std::string &dirName)
{
	...
	ret = GetConfigPath(path, sizeof(path), "Louper/LDE/basic/profiles/");
	if (ret <= 0) {
		blog(LOG_WARNING, "Failed to get profiles config path");
		return false;
	}
...
static bool CopyProfile(const char *fromPartial, const char *to)
{
	...
	ret = GetConfigPath(dir, sizeof(dir), "Louper/LDE/basic/profiles/");
	if (ret <= 0) {
		blog(LOG_WARNING, "Failed to get profiles config path");
		return false;
	}
			  ...
			  const char *init_text, bool rename)
{
    ...
    bool showWizardChecked = false; // config_get_bool(App()->GlobalConfig(), "Basic", "ConfigOnNewProfile");
...
void OBSBasic::on_actionImportProfile_triggered()
{
	...
	int ret = GetConfigPath(path, 512, "Louper/LDE/basic/profiles/");
	if (ret <= 0) {
